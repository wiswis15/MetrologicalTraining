...
static void getPackageName(const char cwd[], const char inputBase[], char packageName[])
{
   int cwdLength = strlen(cwd);
   const char * packageNamePointer = cwd + strlen(inputBase);
   int foundName = 0;
   int currentIndex = strlen(inputBase);

   while (1) {
      if (currentIndex >= cwdLength)
         break;

      if ((*packageNamePointer) != '/') {
         foundName = 1;
         break;
      }
   ...
   if (!foundName) {
      fprintf(stderr, "Failed to find package name: \"%s\", \"%s\"\n", cwd, inputBase);
      strcpy(packageName, "unnamed");
      return;
   }

   const char * nextSlashPointer = strstr(packageNamePointer, "/");
   int packageNameLength = 0;
   if (nextSlashPointer != NULL) {
      packageNameLength = nextSlashPointer - packageNamePointer;
   } else {
      packageNameLength = strlen(packageNamePointer);
   }

   memcpy(packageName, packageNamePointer, packageNameLength);
...
static void logCommandLine(int argc, char ** argv)
{
   ...
   const char * envVarOutputDir = getenv("BR_TOOLCHAIN_OVERRIDE_TRACE_ARGV_PATH");
   if (envVarOutputDir != NULL) {
      outputDir = envVarOutputDir;
   }

   char packageNameDir[1024];
   const char * envVarInputBase = getenv("BR_TOOLCHAIN_OVERRIDE_TRACE_ARGV_INPUT_BASE");
   char  packageName[1024];
   if (envVarInputBase != NULL) {
      if (strstr(envVarInputBase, cwd) != envVarInputBase) {
         getPackageName(cwd, envVarInputBase, packageName);
         sprintf(packageNameDir, "%s/%s", envVarOutputDir, packageName);
         outputDir = packageNameDir;
      } else {
         fprintf(stderr, "Not a valid prefix for tracing argv: \"%s\", \"%s\"\n", cwd, envVarInputBase);
      }
   }

   mkdir(outputDir, 0777);

   sprintf(fileNameBuffer, "%s/%s.%06d", outputDir, timeBuffer, getpid());
...
{
   const char * envVarValue = getenv("BR_TOOLCHAIN_OVERRIDE_IGNORE_ARG");
...
static void editArgs(int * argc, char *** argv)
{
  ...
  if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_TRACE_ARGV")) {
  ...
  int newIndex = 1;
  if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_RECORD_SWITCHES")) {
     newArgv[newIndex] = "-frecord-gcc-switches";
     newIndex++;
  }
  
  if (getenv("BR_TOOLCHAIN_OVERRIDE_OPTIMIZATION_OVERRIDE") != NULL) {
     newArgv[newIndex] = getenv("BR_TOOLCHAIN_OVERRIDE_OPTIMIZATION_OVERRIDE");
     newIndex++;
  }
  
  if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_FORCE_SYMBOLS")) {
     newArgv[newIndex] = "-g";
     newIndex++;
  }
    
  if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_NO_WARNINGS")) {
     newArgv[newIndex] = "-w";
     newIndex++;
  }

  if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_UNWIND_TABLES")) {
     newArgv[newIndex] = "-funwind-tables";
     newIndex++;
  }
  ...
  for (int i = 1; i < orgArgc; i++) {
     ...
     if ((getenv("BR_TOOLCHAIN_OVERRIDE_OPTIMIZATION_OVERRIDE") != NULL) && stringStartsWith(orgArg, "-O"))
        continue;
     
     if (envVarIsSet("BR_TOOLCHAIN_OVERRIDE_FORCE_SYMBOLS") && stringStartsWith(orgArg, "-g"))
        continue;
