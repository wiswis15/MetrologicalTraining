...
namespace Plugin {
    ...
    constexpr uint32 WaitForResponse = 2000;
    ...
    void NTPClient::Initialize(SourceIterator& sources, const uint16 retries, const uint16 delay) {
        ...
        while (sources.Next() == true) {
            ...
            if (url.Type() == Core::URL::SCHEME_NTP) {
                ...
                if (url.Port().IsSet() == true) {
                    ...
                    hostname += ':' + Core::NumberType<uint16>(url.Port().Value()).Text();
                }
    ...
    /* virtual */ uint32 NTPClient::Synchronize()
    {
    ...
    /* virtual */ uint64 NTPClient::SyncTime() const {
    ...
    /* virtual */ uint16 NTPClient::SendData(uint8* dataFrame, const uint16 maxSendSize)
    {
        uint16 result = 0;
    ...
    inline static uint64 SecondsToTicks(double seconds)
    {
        return static_cast<uint64>(seconds * NTPClient::MicroSeconds);
    }

    /* virtual */ uint16 NTPClient::ReceiveData(uint8* dataFrame, const uint16 receivedSize)
    {
        ...
        if (receivedSize == NTPPacket::PacketSize) {
            ...
            uint64 receivedTicks = SecondsToTicks(received);
