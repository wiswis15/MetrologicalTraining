...
namespace WPEFramework
{
    namespace Plugin {
        ...
        void DeviceInfo::SysInfo(Data::SysInfo& systemInfo)
        {
            ...
            systemInfo.Version = _service->Version() + "-" + string(_T(EXPAND_AND_QUOTE(BUILD_REFERENCE)));
            ...
            systemInfo.CpuLoad = Core::NumberType<uint32>(static_cast<uint32>(singleton.GetCpuLoad())).Text();
        ...
        {
            string result;
            const PluginHost::ISystemInfo* info (_service->QueryInterfaceByCallsign<PluginHost::ISystemInfo>(EMPTY_STRING));
