...
namespace Plugin {
	...
	if (config.OutOfProcess.Value() == true) {
            _server = _service->Instantiate<Exchange::IWebServer>(2000, _T("WebServerImplementation"), static_cast<uint32>(~0), _pid, service->Locator());
        }
        else {
            _server = Core::ServiceAdministrator::Instance().Instantiate<Exchange::IWebServer>(Core::Library(), _T("WebServerImplementation"), static_cast<uint32>(~0));
        }
