...
namespace Plugin {
	...
	private:
		...
		public:
			OutgoingChannel(const string& path, const string& replacement, ProxyMap& proxyMap, const Core::NodeId& remoteId)
                ...
                void ProxyRequest(Core::ProxyType<Web::Request>& request, uint32 id)
                {
	...
	public:
            ...
            bool Relay(Core::ProxyType<Web::Request>& request, uint32 channelId)
            {
			...
			while ((found == false) && (index != _proxies.end())) {
				...
				uint32 checkSize(proxyPath.length());
            ...
            inline void Submit(uint32 channelId, Core::ProxyType<Web::Response>& response)
            {
	...
	public:
            inline uint32 Configure (const string& prefixPath, const Config& configuration)
            {
			Core::NodeId accessor;
			uint32 result(Core::ERROR_INCOMPLETE_CONFIG);
			...
			if (listenNode.IsValid() == true) {
				...
				if (configuration.Port.Value() != 80) {
					...
					_accessor += ':' + Core::NumberType<uint32>(configuration.Port.Value()).Text();
				}
		...
		return (Core::SocketServerType<IncomingChannel>::Close(waitTime));
            ...
            inline bool Relay(Core::ProxyType<Web::Request>& request, const uint32 id)
            {
    ...
    public:
		...
		config.FromString(service->ConfigLine());
                ...
                uint32 result(_channelServer.Configure(service->DataPath(), config));
        ...
        virtual uint32 Request(const PluginHost::IStateControl::command state)
        {
