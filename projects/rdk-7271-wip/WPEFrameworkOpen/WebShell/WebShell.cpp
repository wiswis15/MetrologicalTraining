...
#define SLOT_ALLOCATION 8
    ...
    private:
        ...
        public:
            ...
            uint16 Backup(const uint8 data[], const uint16 size) {
                uint16 copySize = (size <= (sizeof(_buffer) - _leftSize) ? size : (sizeof(_buffer) - _leftSize));
            ...
            void Write() {
                uint32 writtenBytes = ::write (_process->Input(), _buffer, _leftSize);
        ...
        static constexpr uint32 MonitorStackSize = 64 * 1024;
    ...
    public:
        ...
        uint32 Read (const uint32 channelId, uint8 data[], const uint16 length) const
        {
            uint32 result = 0;
        ...
        uint32 Write (const uint32 channelId, const uint8 data[], const uint16 length)
        {
            uint32 result = 0;
        ...
        virtual uint32 Worker()
        {
            ...
            if (filledFileDescriptors > 1) {
                ...
                if (result == -1) {
                    ...
                    uint32 VARIABLE_IS_NOT_USED bytes = read(_signalFD, &info, sizeof(info));
    ...
    /* virtual */ uint32 WebShell::Inbound(const uint32 ID, const uint8 data[], const uint16 length)
    {
    ...
    /* virtual */ uint32 WebShell::Outbound(const uint32 ID, uint8 data[], const uint16 length) const
    {
        uint32 result = _sessionMonitor->Read (ID, data, length);
