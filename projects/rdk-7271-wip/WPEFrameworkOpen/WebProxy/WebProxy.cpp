...
ENUM_CONVERSION_END(Core::SerialPort::Parity);
	...
	namespace Plugin {
		...
		public:
			StreamChannel(WebProxy::Connector& parent, const uint32 bufferSize, const Core::NodeId& remote) :
		...
		public:
			DatagramChannel(WebProxy::Connector& parent, const uint32 bufferSize, const Core::NodeId& remote) :
		...
		public:
			...
			{
				uint32 result = BaseClass::Open(waitTime);
		...
		public:
			...
			#endif
			inline ConnectorWrapper(PluginHost::Channel& channel, const uint32 bufferSize)
				: WebProxy::Connector(channel, &_streamType)
				, _streamType(*this, bufferSize)
			{
			}
			inline ConnectorWrapper(PluginHost::Channel& channel, const uint32 bufferSize, const Core::NodeId& remoteId)
				: WebProxy::Connector(channel, &_streamType)
		...
		/* virtual */ bool WebProxy::Attach(PluginHost::Channel& channel)
		{
			...
			std::map<const uint32, Connector*>::iterator connection(_connectionMap.begin());
			...
			if (_connectionMap.size() < _maxConnections) {
				Connector* newLink = CreateConnector(channel);
					...
					if (newLink != NULL) {
						_connectionMap.insert(std::pair<uint32, Connector*>(channel.Id(), newLink));
			...
			/* virtual */ void WebProxy::Detach(PluginHost::Channel& channel)
			{
				...
				std::map<const uint32, Connector*>::iterator connection = _connectionMap.find(channel.Id());
			...
			/* virtual */ uint32 WebProxy::Inbound(const uint32 ID, const uint8 data[], const uint16 length)
			{
				uint32 result = length;

				// See if we can forward this info..
				std::map<const uint32, Connector*>::iterator connection = _connectionMap.find(ID);
			...
			/* virtual */ uint32 WebProxy::Outbound(const uint32 ID, uint8 data[], const uint16 length) const
			{
				uint32 result = 0;

				// See if we can forward this info..
				std::map<const uint32, Connector*>::const_iterator connection = _connectionMap.find(ID);
			...
			WebProxy::Connector* WebProxy::CreateConnector(PluginHost::Channel& channel) const
			{
				...
				WebProxy::Connector* result(NULL);
