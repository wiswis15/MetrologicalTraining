	...
	namespace Plugin {
		...
		public:
			...
			public:
				...
				{
					return ((_channel == NULL) && (_link->IsClosed()));
				}
				...
				uint16 SendData(uint8* dataFrame, const uint16 maxSendSize)
				{
					...
					uint16 result = _socketBuffer.Read(dataFrame, maxSendSize);
				...
				uint16 ReceiveData(uint8* dataFrame, const uint16 receivedSize)
				{
					...
					uint16 result = _channelBuffer.Write(dataFrame, receivedSize);
				...
				uint16 ChannelSend(uint8* dataFrame, const uint16 maxSendSize) const
				{
					_adminLock.Lock();

					uint16 result = _channelBuffer.Read(dataFrame, maxSendSize);
				...
				uint16 ChannelReceive(const uint8* dataFrame, const uint16 receivedSize)
				{
					_adminLock.Lock();

					bool wasEmpty = _socketBuffer.IsEmpty();

					uint16 result = _socketBuffer.Write(dataFrame, receivedSize);
			...
			public:
				...
				public:
					...
					public:
						...
						Settings(const uint32 baudRate, const Core::SerialPort::Parity parity, const uint8 bits, const uint8 stopbits)
							: Core::JSON::Container()
						{
							Add(_T("baudrate"), &Baudrate);
							Add(_T("parity"), &Parity);
				...
				public:
					...
					Link(const string& name, const enumType type, const bool text, const string device, const uint32 baudRate, const Core::SerialPort::Parity parity, const uint8 bits, const uint8 stopbits)
						: Core::JSON::Container()
					{
						Add(_T("name"), &Name);
						Add(_T("type"), &Type);
		...
		public:
			...
			virtual uint32 Inbound(const uint32 ID, const uint8 data[], const uint16 length);
			...
			virtual uint32 Outbound(const uint32 ID, uint8 data[], const uint16 length) const;
		...
		private:
			...
			std::map<const uint32, Connector*> _connectionMap;
