...
{
	uint32 attention (static_cast<CustomLoopHandler*>(mySource)->attentionPending);
...
--------------------------------------------------------------------------------------------------- */
    ...
    public:
        ...
        virtual uint32 Request(PluginHost::IStateControl::command command)
        {
        ...
        virtual uint32 Configure(PluginHost::IShell* service)
        {
            ...
            const char* queuePlayerFlushMode = getenv("QUEUEPLAYER_FLUSH_MODE");

            Core::SystemInfo::SetEnvironment(_T("QUEUEPLAYER_FLUSH_MODE"), queuePlayerFlushMode ? _T(queuePlayerFlushMode) : _T("3"));
            Core::SystemInfo::SetEnvironment(_T("HOME"), service->PersistentPath());
            Core::SystemInfo::SetEnvironment(_T("WPE_CALLSIGN"), service->Callsign());

            // Set dummy window for gst-gl
            Core::SystemInfo::SetEnvironment(_T("GST_GL_WINDOW"), _T("dummy"));
            ...
            if (_config.MemoryPressure.Value().empty() == false)
                Core::SystemInfo::SetEnvironment(_T("WPE_POLL_MAX_MEMORY"), _config.MemoryPressure.Value());
            ...
            if (_config.Inspector.Value().empty() == false)
                Core::SystemInfo::SetEnvironment(_T("WEBKIT_INSPECTOR_SERVER"), _config.Inspector.Value());
            ...
            if (_config.JavaScript.DumpOptions.Value().empty() == false) {
                Core::SystemInfo::SetEnvironment(_T("JSC_dumpOptions"), _config.JavaScript.DumpOptions.Value());
            }
    ...
    private:
            ...
            if (_context != NULL) {
                ...
                g_main_context_invoke(
                    ...
                    [](gpointer customdata) -> gboolean {
                        ...
                        object->Hidden(true);
			#if _TRACE_LEVEL > 0
                        uint32 time = static_cast<uint32>(Core::Time::Now().Ticks() - object->_time);
                        TRACE_L1 ("Internal Hide Notification took %d mS.", time);
			#endif
                        return FALSE;
                    },
                    this);
        ...
        void Show()
        {
			...
			#if _TRACE_LEVEL > 0
                        uint32 time = static_cast<uint32>(Core::Time::Now().Ticks() - object->_time);
                        TRACE_L1("Internal Show Notification took %d mS.", time);
			#endif
                        return FALSE;
                    },
                    this);
        ...
        void Suspend()
        {
			...
			#if _TRACE_LEVEL > 0
                        uint32 time = static_cast<uint32>(Core::Time::Now().Ticks() - object->_time);
                        TRACE_L1("Internal Suspend Notification took %d mS.", time);
			#endif
        ...
        void Resume()
        {
			...
			#if _TRACE_LEVEL > 0
                        uint32 time = static_cast<uint32>(Core::Time::Now().Ticks() - object->_time);
                        TRACE_L1("Internal Resume Notification took %d mS.", time);
			#endif
...
namespace WebKitBrowser {
    ...
    public:
        ...
        {
            const uint8 requiredChildren = (sizeof(mandatoryProcesses) / sizeof(mandatoryProcesses[0]));
            ...
            uint32 requiredProcesses = (0xFFFFFFFF >> (32 - requiredChildren));
    ...
    private:
        ...
        {
            return (Core::Time::Now().Ticks() < _startTime);
        }

    private:
        Core::ProcessInfo _main;
        mutable Core::ProcessInfo::Iterator _children;
        uint64 _startTime; // !< Reference for monitor
    };
