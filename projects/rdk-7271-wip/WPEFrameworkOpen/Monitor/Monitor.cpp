...
namespace Plugin {
    ...
    static Core::ProxyPoolType<Web::JSONBodyType<Monitor::Data::Memory> > jsonMemoryBodyDataFactory(2);
    ...
    /* virtual */ const string Monitor::Initialize(PluginHost::IShell* service)
    {
        ...
        Core::JSON::ArrayType<ObserveInfo>::Iterator index(_config.Observables.Elements());
        ...
        if (request.Verb == Web::Request::HTTP_GET) {
            ...
            if (index.Next() == false) {
                ...
                if (_monitor->Snapshot(index.Current().Text(), memoryInfo) == true) {
                    Core::ProxyType<Web::JSONBodyType<Monitor::Data::Memory> > response(jsonMemoryBodyDataFactory.Element());
