...
//WPEFramework::Core::SHA1 hash1(reinterpret_cast<const uint8*>(&__HASH_1__[0]), (sizeof(__HASH_1__) - 1));
//const uint8* hashCode = hash1.Result();
//for (uint8 teller = 0; teller < 20; teller++)
//{
//    printf ("%02X", hashCode[teller]);
//}
//printf("\n");
//string baseHash1;
//WPEFramework::Core::ToString(hash1.Result(), 5 * sizeof(uint32), true, baseHash1);
//printf("Hash1 = %s\n", baseHash1.c_str());

//// Lets see if we can translate it back
//uint8 buffer[24];
//uint16 bufferLength = sizeof(buffer);
//uint16 location = WPEFramework::Core::FromString(baseHash1, buffer, bufferLength);
...
//WPEFramework::Core::SHA1 hash2(reinterpret_cast<const uint8*>(&__HASH_2__[0]), (sizeof(__HASH_2__)-1));
//string baseHash2;
//WPEFramework::Core::ToString(hash2.Result(), 5 * sizeof(uint32), true, baseHash2);
//printf("Hash2 = %s\n", baseHash2.c_str());

//// HASH3 => da39a3ee 5e6b4b0d 3255bfef 95601890 afd80709
//// Base64 => 2jmj7l5rSw0yVb/vlWAYkK/YBwk=
//WPEFramework::Core::SHA1 hash3(reinterpret_cast<const uint8*>(&__HASH_3__[0]), (sizeof(__HASH_3__)-1));
//string baseHash3;
//WPEFramework::Core::ToString(hash3.Result(), 5 * sizeof(uint32), true, baseHash3);
//printf("Hash3 = %s\n", baseHash3.c_str());
...
#pragma warning(disable : 4355)
    ...
    // Scope to make sure that the sockets are destructed before we close the singletons !!!
    {
        ...
        for (uint8 engine = 0; engine < STRESS_ENGINES; engine++) {
        ...
        sint32 randomNumber = 4526;
        // WPEFramework::Core::Random<sint32> (randomNumber, -70000, +70000);
        ...
        do {
            ...
            case '0': {
                if (openState == false) {
                    ...
                    uint32 result = webSocketRemoteConnection.Open(1000);
                    SleepMs(1000);
            ...
            case 'T': {
                ...
                uint32 minSize = static_cast<uint32>(~0);
                uint32 maxSize = 0;
                uint32 pendingMessages = 0;
                uint32 errors = 0;
                uint32 receivedSize = 0;
                uint32 sendSize = 0;
                uint32 sendMessages = 0;
