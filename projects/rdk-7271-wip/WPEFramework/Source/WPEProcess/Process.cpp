...
#include "Module.h"
#include "../rpc/ObjectMessageHandler.h"
...
namespace Process {
    ...
    public:
        ...
        void ProcessProcedures()
        {
            ...
            while ((admin.Instances() > 0) && (_handleQueue.Extract(newRequest, Core::infinite) == true)) {
                ...
                // uint8 methodId (parameters.MethodId());
                // uint32 interfaceId(parameters.InterfaceId());
    ...
    private:
        virtual void Option(const TCHAR option, const TCHAR* argument)
        {
            ...
            case 'm':
                ...
                break;
            case 'i':
                InterfaceId = Core::NumberType<uint32>(Core::TextFragment(argument)).Value();
                break;
            case 'v':
                Version = Core::NumberType<uint32>(Core::TextFragment(argument)).Value();
                break;
            case 'h':
            default:
                RequestUsage(true);
    ...
    static void* CheckInstance(const TCHAR* path, const TCHAR locator[], const TCHAR className[], const uint32 ID, const uint32 version)
    {
        void* result = nullptr;
...
#endif
{
    ...
    if ((options.RequestUsage() == true) || (options.Locator == nullptr) || (options.ClassName == nullptr) || (options.RemoteChannel == nullptr)) {
        ...
        printf("This application spawns a seperate process space for a WPEFramework plugin. The WPEFramework ");
        printf("plugins are searched in the same order as they are done in process. Starting from:\n");
        ...
        for (uint8 teller = 0; teller < argc; teller++) {
