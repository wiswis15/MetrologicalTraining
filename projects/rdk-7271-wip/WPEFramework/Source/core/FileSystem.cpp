...
namespace Core {
    ...
    void File::LoadFileInfo()
    {
        ...
        if (stat(_name.c_str(), &data) == 0) {
            ...
            _attributes |= ((data.st_mode & S_IFDIR) == S_IFDIR ? FILE_DIRECTORY : 0);
            _attributes |= ((data.st_mode & S_IFREG) == S_IFREG ? FILE_NORMAL : 0);
            _attributes |= ((data.st_mode & S_IFLNK) == S_IFLNK ? FILE_LINK : 0);
...
#ifdef __WIN32__
            for (uint32 teller = 0; teller < length; teller++) {
                if (result[teller] == '\\') {
                    result[teller] = '/';
                }
            }
