...
//----------------------------------------------------------------------------
...
#else
        uint32 nResult = Core::ERROR_NONE;
    ...
    CountingSemaphore::Lock()
    {
        ...
        uint32 nResult = m_syncMinLimit.Lock();
...
#else
        // First see if we could still decrease the count..
        uint32 nResult = m_syncMinLimit.Lock(nMilliSeconds);
    ...
    CountingSemaphore::Unlock(unsigned int nCount)
    {
        ...
        uint32 nResult = m_syncMaxLimit.Lock(Core::infinite);
...
#ifdef __POSIX__
    uint32 CountingSemaphore::TryUnlock(unsigned int nMilliSeconds)
#else
    uint32 CountingSemaphore::TryUnlock(unsigned int /* nMilliSeconds */)
#endif
    {
#ifdef __POSIX__
        // First see if we could still increase the count..
        uint32 nResult = m_syncMaxLimit.Lock(nMilliSeconds);
    ...
    CriticalSection CriticalSection::_StdErrDumpMutex;
		...
		uint32 DoorBell::Wait(const uint32 waitTime) const {

			uint32 result = Core::ERROR_GENERAL;
            if (_doorBell != nullptr) {
#ifdef __WIN32__
