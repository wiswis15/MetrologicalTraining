...
namespace Core {
    ...
    static ConsoleLogging InternalDefaultLogger(NULL, llDebug);
    ILogService* DefaultLogger = &InternalDefaultLogger;
    static uint32 StartLogTimeStamp = static_cast<uint32>(Core::Time::Now().Ticks());

    static const string FormatLogLine(uint32 timeStamp, const string& className, const string& message)
    {
        static uint32 SequenceNumber = 0;
        ...
        char buf[256];
        uint32 relativeTime = timeStamp - StartLogTimeStamp;
        uint32 sec = relativeTime / 1000;
        uint32 ms = relativeTime % 1000;
    ...
    {
        uint32 stamp = static_cast<uint32>(Core::Time::Now().Ticks());
    ...
    /* virtual */ void ConsoleLogging::Log(const uint32 timeStamp, const LogLevel level, const string& className, const string& msg)
    {
    ...
    /* virtual */ void SocketLogging::Log(const uint32 timeStamp, const LogLevel level, const string& className, const string& msg)
    {
        if (m_NextLogger != NULL) {
            m_NextLogger->Log(timeStamp, level, className, msg);
        }
        ...
        if (level >= m_LogLevel) {
            ...
            try {
                uint32 size = logLine.length();

                // Repeatedly send the std::string (not including \0) to the server
                //m_Socket.Write(size, reinterpret_cast<const uint8*>(logLine.c_str()), 100);
            }
...
#pragma warning(default : 4355)
    ...
    /* virtual */ void DecoupledLogging::Log(const uint32 timeStamp, const LogLevel level, const string& className, const string& msg)
    {
