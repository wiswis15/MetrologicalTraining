...
namespace Core {
    ...
    /* static */ uint32 Thread::_defaultStackSize = 0;

    Thread::Thread(const uint32 stackSize, const TCHAR* threadName)
        : m_enumState(BLOCKED)
        , m_syncAdmin()
...
// Create a worker that can do actions in parallel
        ...
        m_ThreadId = (uint32)(size_t)m_hThreadInstance;
#endif
