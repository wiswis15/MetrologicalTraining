...
namespace Core {
    ...
    public:
        ...
        inline void PassThrough(const uint16 passThroughBytes)
        {
        ...
        uint16 Deserialize(const uint8 stream[], const uint16 maxLength)
        {
            ...
            while (current < maxLength) {
                ...
                while (((_state & EXTERNALPASS) != 0) && (current < maxLength)) {
                    uint16 passOn = ((maxLength - current) > _byteCounter ? _byteCounter : (maxLength - current));
                ...
                while (((_state & FLUSH_LINE) != 0) && (current < maxLength)) {
                    sint8 terminated = _terminator.IsTerminated(stream[current]);
                ...
                while (((_state & SKIP_WHITESPACE) == 0) && (current < maxLength)) {
                    ...
                    if ((_state & ESCAPED) == ESCAPED) {
                        ...
                        sint8 terminated = _terminator.IsTerminated(character);
    ...
    public:
        ...
        void ReadNumber(OptionalType<NUMBER>& result, const NumberBase type = BASE_DECIMAL)
        {
            ...
            // Read the number..
            uint32 readChars = NumberType<NUMBER, SIGNED>::Convert(&(Data()[marker]), Length() - marker, value, type);
        ...
        void ReadRange(OptionalType<RangeType<NUMBER, BEGININC, ENDINC> >& result, const NumberBase type = BASE_DECIMAL)
        {
            ...
            uint32 readBytes = NumberType<NUMBER, SIGNED>::Convert(Data(), Length(), minimum, type);
            ...
            if (Data()[readBytes] == DIVIDER) {
                ...
                uint32 handled = NumberType<NUMBER, SIGNED>::Convert(&(Data()[readBytes]), Length() - readBytes, maximum, type);
