...
namespace Core {
    ...
    void ProcessName(const uint32 pid, TCHAR buffer[], const uint32 maxLength)
    {
    ...
    void ProcessName(const uint32 pid, TCHAR buffer[], const uint32 maxLength)
    {
        ...
        sint32 length = readlink(buffer, buffer, maxLength - 1);
    ...
    static string ExecutableName(const uint32 PID)
    {
    ...
    static void FindChildren(const uint32 parent, std::list<uint32>& children)
    {
        DIR* dp;
        struct dirent* ep;
                ...
                if ('\0' == endptr[0]) {
                    ...
                    if ((fd = open(buffer, O_RDONLY)) > 0) {
                        if (read(fd, buffer, sizeof(buffer)) > 0) {
                            ...
                            if (static_cast<uint32>(ppid) == parent) {
    ...
    // Iterate over Processes
    static void FindPid(const string& item, const bool exact, std::list<uint32>& pids)
    {
        DIR* dp;
        struct dirent* ep;
            ...
            if (Handle) {
                ...
                if (::GetModuleFileNameEx(Handle, 0, Buffer, MAX_PATH)) {
                    if (name == Buffer) {
                        ...
                        _pids.push_back(static_cast<uint32>(processInfo.th32ProcessID));
                    }
                }
                CloseHandle(Handle);
            }
    ...
    ProcessInfo::Iterator::Iterator(const uint32 parentPID)
    {
#ifdef __WIN32__
        HANDLE hSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
        PROCESSENTRY32 processInfo;
        ...
        while (Process32Next(hSnapShot, &processInfo) != FALSE) {
            if (static_cast<uint32>(processInfo.th32ParentProcessID) == parentPID) {
                // Add this entry to the list
                _pids.push_back(static_cast<uint32>(processInfo.th32ProcessID));
            }
        }
    ...
    ProcessInfo::ProcessInfo(const uint32 id)
        : _pid(id)
#ifdef __WIN32__
        , _handle(OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, _pid))
#endif
    ...
    uint64 ProcessInfo::Allocated() const
    {
        uint64 result = 0;
    ...
    uint64 ProcessInfo::Resident() const
    {
        uint64 result = 0;
    ...
    uint64 ProcessInfo::Shared() const
    {
        uint64 result = 0;
