...
#define __STREAMTYPELENGTHVALUE_H
...
namespace Core {
    ...
    public:
        inline TypeLengthValueType(const TYPE& type, const LENGTH& length, const uint8* value)
            : _type(type)
            , _length(length)
            , _value(value)
        {
    ...
    public:
        ...
        inline const uint8* Value() const
        {
            return (_value);
        }
    ...
    template <typename SOURCE, typename TYPE, typename LENGTH, const uint32 DEFAULTVALUESIZE>
    ...
    public:
        ...
        StreamTypeLengthValueType()
            ...
            , _sendBuffer(new uint8[_sendMaxSize])
            , _sendSize(0)
            , _receiveMaxSize(DEFAULTVALUESIZE)
            , _receiveBuffer(new uint8[_receiveMaxSize])
            , _receiveSize(0)
            , _adminLock()
        {
        }
    ...
    public:
        ...
        inline uint32 Open(uint32 waitTime)
        {
            return (_channel.Open(waitTime));
        }
        inline uint32 Close(uint32 waitTime)
        {
        ...
        void Submit(const TypeLengthValueType<TYPE, LENGTH>& element)
        {
            ...
            if ((_sendSize + element.Length() + sizeof(TYPE) + sizeof(LENGTH)) > _sendMaxSize) {
                ...
                uint8* newBuffer = new uint8[_sendMaxSize];
    ...
    private:
        ...
        uint16 SendData(uint8* dataFrame, const uint16 maxSendSize)
        {
            ...
            while ((result < maxSendSize) && (_sendSize > 0)) {
                uint32 chunkLeft = ((_sendLength + sizeof(TYPE) + sizeof(LENGTH)) - _sendOffset);
                uint16 chunkSize = (maxSendSize > chunkLeft ? chunkLeft : maxSendSize);
                ...
                if (chunkSize != chunkLeft) {
                    ...
                    uint32 totalLength = _sendLength + sizeof(TYPE) + sizeof(LENGTH);

                    // We send it all. Report and move on..
                    Send(TypeLengthValueType<TYPE, LENGTH>(_sendType, _sendLength, (_sendLength > 0 ? _sendBuffer : NULL)));
        ...
        uint16 ReceiveData(uint8* dataFrame, const uint16 availableData)
        {
            ...
            while (result != availableData) {
                if (_receiveSize < sizeof(TYPE)) {
                    ...
                    uint8* buffer = static_cast<uint8*>(&_receiveType);
                ...
                if ((_receiveSize >= sizeof(TYPE)) && (_receiveSize < (sizeof(TYPE) + sizeof(LENGTH)))) {
                    uint8* buffer = static_cast<uint8*>(&_receiveLength);
                ...
                if (_receiveSize >= (sizeof(TYPE) + sizeof(LENGTH))) {
                    ...
                    if ((_receiveSize - sizeof(TYPE) - sizeof(LENGTH)) == _receiveLength) {
                        Received(TypeLengthValueType<TYPE, LENGTH>(_receiveType, _receiveLength, (_receiveLength > 0 ? _receiveBuffer : NULL)));
...
#endif // __STREAMTYPELENGTHVALUE_H
