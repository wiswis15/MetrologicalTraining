...
namespace Core {
    namespace JSON {
        ...
        struct EXTERNAL IDirect {
            ...
            virtual uint16 Serialize(char Stream[], const uint16 MaxLength, uint16& offset) const = 0;
            virtual uint16 Deserialize(const char Stream[], const uint16 MaxLength, uint16& offset) = 0;
        };
        ...
        struct EXTERNAL IElement {
            ...
            static bool FromString(const string& text, INSTANCEOBJECT& realObject)
            {
                ...
                while ((ready == false) && (fillCount < text.size())) {
                    uint8 buffer[1024];
                    uint16 size = ((text.size() - fillCount) < sizeof(buffer) ? (static_cast<uint16>(text.size()) - fillCount) : sizeof(buffer));
            ...
            static bool ToFile(Core::File& fileObject, const INSTANCEOBJECT& realObject)
            {
                ...
                if (fileObject.IsOpen()) {
                    ...
                    while ((ready == false) && (missedBytes == 0)) {
                        uint8 buffer[1024];
                        uint16 bytes = serializer.Serialize(buffer, sizeof(buffer));
            ...
            static bool FromFile(Core::File& fileObject, INSTANCEOBJECT& realObject)
            {
                ...
                if (fileObject.IsOpen() == true) {
                    ...
                    while ((ready == false) && (unusedBytes == 0) && (readBytes != 0)) {
                        ...
                        readBytes = static_cast<uint16>(fileObject.Read(buffer, sizeof(buffer)));
        ...
        typedef NumberType<uint8, false, BASE_DECIMAL> DecUInt8;
        typedef NumberType<sint8, true, BASE_DECIMAL> DecSInt8;
        typedef NumberType<uint16, false, BASE_DECIMAL> DecUInt16;
        typedef NumberType<sint16, true, BASE_DECIMAL> DecSInt16;
        typedef NumberType<uint32, false, BASE_DECIMAL> DecUInt32;
        typedef NumberType<sint32, true, BASE_DECIMAL> DecSInt32;
        typedef NumberType<uint64, false, BASE_DECIMAL> DecUInt64;
        typedef NumberType<sint64, true, BASE_DECIMAL> DecSInt64;
        typedef NumberType<uint8, false, BASE_HEXADECIMAL> HexUInt8;
        typedef NumberType<sint8, true, BASE_HEXADECIMAL> HexSInt8;
        typedef NumberType<uint16, false, BASE_HEXADECIMAL> HexUInt16;
        typedef NumberType<sint16, true, BASE_HEXADECIMAL> HexSInt16;
        typedef NumberType<uint32, false, BASE_HEXADECIMAL> HexUInt32;
        typedef NumberType<sint32, true, BASE_HEXADECIMAL> HexSInt32;
        typedef NumberType<uint64, false, BASE_HEXADECIMAL> HexUInt64;
        typedef NumberType<sint64, true, BASE_HEXADECIMAL> HexSInt64;
        typedef NumberType<uint8, false, BASE_OCTAL> OctUInt8;
        typedef NumberType<sint8, true, BASE_OCTAL> OctSInt8;
        typedef NumberType<uint16, false, BASE_OCTAL> OctUInt16;
        typedef NumberType<sint16, true, BASE_OCTAL> OctSInt16;
        typedef NumberType<uint32, false, BASE_OCTAL> OctUInt32;
        typedef NumberType<sint32, true, BASE_OCTAL> OctSInt32;
        typedef NumberType<uint64, false, BASE_OCTAL> OctUInt64;
        typedef NumberType<sint64, true, BASE_OCTAL> OctSInt64;
        ...
        private:
            ...
            static constexpr uint8 ValueBit = 0x01;
            static constexpr uint8 DefaultBit = 0x02;
            static constexpr uint8 SetBit = 0x04;
        ...
        private:
            static constexpr uint32 None = 0x00000000;
            static constexpr uint32 ScopeMask = 0x1FFFFFFF;
            static constexpr uint32 QuotedSerializeBit = 0x80000000;
            static constexpr uint32 SetBit = 0x40000000;
            static constexpr uint32 QuoteFoundBit = 0x20000000;
        ...
        private:
            ...
            virtual uint16 Serialize(char stream[], const uint16 maxLength, uint16& offset) const override
            {
                uint16 result = 0;
...
#pragma warning(disable : 4996)
                    ...
                    uint16 written = static_cast<uint16>(_value.copy(&(stream[result]), maxLength - result, offset - 1));
...
#pragma warning(default : 4996)
            ...
            virtual uint16 Deserialize(const char stream[], const uint16 maxLength, uint16& offset) override
            {
                bool finished = false;
                uint16 result = 0;
                ASSERT(maxLength > 0);
                ...
                if (finished == false) {
                    offset =static_cast<uint16>( _value.length() + (((_scopeCount & ScopeMask) != 0) ? 1 : 0));
                } else {
                    offset = 0;
        ...
        private:
            ArrayType<ELEMENT>& operator=(const ArrayType<ELEMENT>& RHS) = delete;
        ...
        public:
            ...
            {
                return static_cast<uint16>(_data.size());
            }
