...
public:
	...
	IPCChannelClientType (const NodeId& node, const uint32 bufferSize)
        ...
        , _factory (ProxyType< FactoryType<IIPC, uint32> >::Create()) {
	...
	IPCChannelClientType (const NodeId& node, const uint32 bufferSize, ProxyType< FactoryType<IIPC, uint32> >& factory)
        : IPCChannelType<SocketPort, EXTENSION>((LISTENING ? SocketPort::LISTEN : SocketPort::STREAM), (LISTENING ? node : node.AnyInterface()), (LISTENING ? node.AnyInterface() : node), bufferSize, bufferSize)
        , _factory (factory) {
		...
		template <const bool FACTORY = INTERNALFACTORY, EnableIfParameter<FACTORY == false> = 0 >
        IPCChannelClientType(const NodeId& node, const uint32 bufferSize, ProxyType<FactoryType<IIPC, uint32> >& factory, SOCKET newSocket)
    ...
    private:
        ProxyType<FactoryType<IIPC, uint32> > _factory;
    };
    ...
    public:
		template <const bool FACTORY = INTERNALFACTORY, EnableIfParameter<FACTORY == true> = 0 >
		IPCChannelServerType(const NodeId& node, const uint32 bufferSize)
            : SocketListner(node)
            , _adminLock()
            , _factory(ProxyType<FactoryType<IIPC, uint32> >::Create())
            , _clients()
        ...
        if (node.IsValid() == true) {
            if (node.Type() == NodeId::TYPE_DOMAIN) {
                ...
                _connector = SystemInfo::Instance().GetHostName() + ':' + NumberType<uint16>(node.PortNumber()).Text();
            } else {
				_connector = node.HostAddress() + ':' + NumberType<uint16>(node.PortNumber()).Text();
			}
        }
    }
	template <const bool FACTORY = INTERNALFACTORY, EnableIfParameter<FACTORY == false> = 0 >
	IPCChannelServerType(const NodeId& node, const uint32 bufferSize, ProxyType< FactoryType<IIPC, uint32> >& factory) :
        SocketListner(node),
        _adminLock(),
        _factory(factory),
        _clients(),
    ...
    virtual ~IPCChannelServerType() {
         TRACE_L1("Closing server in Process. %d", ProcessInfo().Id());
            ...
            if (_clients.size() > 0) {
                ...
                TRACE_L1("Closing clients that should have been closed before destruction [%d].", static_cast<uint32>(_clients.size()));
                CloseClients();
            }
    ...
    public:
        inline Core::ProxyType<Client> operator[](const uint32 index)
        {
        ...
        inline Core::ProxyType<const Client> operator[](const uint32 index) const
        {
        ...
        uint32 Invoke(ProxyType<IIPC>& command, const uint32 waitTime)
        {
    ...
    private:
        ...
        uint32 CallRecursive(typename ClientMap::iterator& index, ProxyType<IIPC>& command, const uint32 waitTime)
        {
            uint32 result = Core::ERROR_NONE;
            ...
            if (index != _clients.end()) {
                ...
                index++;
                uint32 status = CallRecursive(index, command, waitTime);
        ...
        inline std::list<ProxyType<IIPCServer> >::iterator FindHandler(const uint32 id)
        {
