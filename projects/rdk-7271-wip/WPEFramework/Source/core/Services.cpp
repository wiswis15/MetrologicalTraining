...
namespace Core {
    ...
    void* ServiceAdministrator::Instantiate(const Library& library, const char name[], const uint32 version, const uint32 interfaceNumber)
    {
        bool found = false;
        ...
        while ((index != _services.end()) && (found == false)) {
            ...
            found = ((strcmp(thisName, name) == 0) && ((version == static_cast<uint32>(~0)) || (version == (*index)->Version())));

            if (found == false) {
                index++;
            }
        }
        return (found == true ? (*index)->Create(library, interfaceNumber) : NULL);
    }
