...
namespace Core {
    ...
    public:
        HeapStorage(const uint16 length)
            : _length(length)
        ...
        HeapStorage(const HeapStorage& copy)
            ...
            , _buffer(new uint8[copy._length])
        {
        }
        ~HeapStorage()
        {
    ...
    public:
        ...
        {
            ASSERT((_buffer != NULL) && (index < _length));
    ...
    public:
        StackStorage(const uint16 length)
        {
    ...
    public:
        ...
        public:
            ...
            IteratorType(const COPYSTORAGE& copy, const uint16 length)
                : _length(length)
                , _index(0xFFFF)
                , _buffer(copy)
            {
    ...
    public:
        OIDType(const uint8 identifier[], const uint16 length)
            : _buffer(length)
        {
            CopyBuffer(identifier, length);
        }
        OIDType(const string& OID, const uint16 bufferLength = 256)
    ...
    private:
        inline void CopyBuffer(const uint8 identifier[], const uint16 length)
        {
    ...
    public:
        ...
        ASN1SequenceType(const uint8 buffer[], const uint16 length)
            : _buffer(length)
            , _index(0)
            , _length(0)
        {
