...
namespace Core {
    ...
    private:
        ...
        public:
            inline static uint32 CaptureSize()
            {
                return ((BLOCKSIZE * 1024) - (2 * sizeof(Absolute)));
            }
        ...
        protected:
            inline void Start(const uint32 id)
            {
                _start._id = id;
                _start._time = 0;
                _start._value = 0;
            ...
            uint32 Store(const uint64 time, const STOREVALUE value)
            {
                if (_start._time == 0) {
                    ASSERT(_currentDelta == 0);
                    ASSERT((_currentId != 0) && (_currentId != static_cast<uint32>(~0)));
                    ...
                    uint32 delta = static_cast<uint32>((time - _currentTime) / 1000);
            ...
            void StepForward()
            {
                ...
                // Adjust the time
                if (time == true) {
                    ASSERT(delta <= NUMBER_MAX_UNSIGNED(uint32));

                    if (negative) {
                        _currentDelta -= static_cast<uint32>(delta);
                    }
                    else {
                        _currentDelta += static_cast<uint32>(delta);
                    }
            ...
            void StepBack()
            {
                ...
                if (_currentId == _start._id) {
                    ...
                    uint32 handled = BackwardElement(time, negative, delta);
            ...
            void SnapShot(BaseRecorder& copy, const uint32 id) const
            {
                // Copy all recordings..
            ...
            void Load(Core::File& file, Absolute& end, const uint32 id)
            {
                ...
                if (file.Size() <= (2 * sizeof(Absolute))) {
                    ...
                    LoadScalar<uint32>(file, _start._id);
                    LoadScalar<STOREVALUE>(file, _start._value);
                    LoadScalar<uint64>(file, _start._time);

                    LoadScalar<uint32>(file, end._id);
                    LoadScalar<STOREVALUE>(file, end._value);
                    LoadScalar<uint64>(file, end._time);

                    uint32 dataSize = fileSize - file.Position();
            ...
            void Save(Core::File& file)
            {
                ...
                StoreScalar<uint32>(file, _start._id);
                StoreScalar<STOREVALUE>(file, _start._value);
                StoreScalar<uint64>(file, _start._time);

                // Save our end point
                StoreScalar<uint32>(file, _currentId);
                StoreScalar<STOREVALUE>(file, _currentValue);
                StoreScalar<uint64>(file, _currentTime);
        ...
        private:
            uint32 StoreDelta(const bool time, const bool negative, const uint64 input)
            {
                uint64 value = input;
                uint32 addedBytes = 1;

                // For the first byte, we can only have 5 information bits, find out if VALUE fits..
                uint8 current = (time ? 0x40 : 0) | (negative ? 0x20 : 0) | (value > 32 ? 0x80 : 0) | (value & 0x1F);
            ...
            uint32 ForwardElement(bool& time, bool& negative, uint64& value)
            {
                uint32 readBytes = 1;
                uint8 digit;
                uint8 shiftValue = 5;
            ...
            uint32 BackwardElement(bool& time, bool& negative, uint64& result)
            {
                uint32 stepBack = 1;
            ...
            uint32 StoreScalar(File& file, const SCALAR value)
            {
                uint32 length = 0;

                for (unsigned int index = sizeof(SCALAR); index > 0; index--) {
                    uint8 digit = (value >> ((index - 1) << 3)) & 0xFF;
            ...
            template <typename SCALAR>
            uint32 LoadScalar(const Core::File& file, SCALAR& value) const
            {
                uint32 length = 0;
    ...
    public:
        ...
        protected:
                ...
                , _storageName(fileName)
            {
                ...
                if (_fileId == static_cast<uint32>(~0)) {
                    _fileId = 0;
                    startPoint = 1;
                }
                else {
                    ...
                    if (file.Open(true) == true) {
                        ...
                        BaseRecorder::Load(file, end, static_cast<uint32>(~0));
        ...
        public:
            ...
            {
                ProxyType<Writer> result = ProxyType<Writer>::CreateEx(BaseRecorder::CaptureSize(), filename);

                result->SetBuffer(result.Buffer());
        ...
        public:
            ...
            void Record(const STOREVALUE value)
            {
                ...
                uint64 time = Core::Time::Now().Ticks();

                _lock.Lock();

                uint32 size = BaseRecorder::Store(time, value);

                _lock.Unlock();

                if (size >= (BaseRecorder::CaptureSize() - ((8 * (sizeof(STOREVALUE) + sizeof(uint64))) / 6))) {
                    Save();
                }
            }
            void Copy(BaseRecorder& copy, const uint32 id) const
            {
            ...
            void Save()
            {
                ...
                NumberType<uint32> number(_fileId);
        ...
        public:
            Reader(const ProxyType<Writer>& recorder, const uint32 id = static_cast<uint32>(~0))
                : BaseRecorder()
                , _storageName(recorder->Source())
                , _currentSet(recorder)
            {
                _buffer = new uint8[BaseRecorder::CaptureSize()];
                BaseRecorder::SetBuffer(_buffer);
                Reset(id);
            }
            Reader(const string& fileName, const uint32 id = 0)
                : BaseRecorder()
                , _storageName(fileName)
                , _currentSet()
            {
        ...
        public:
            inline uint32 StartId() const
            {
                return (BaseRecorder::Start()._id);
            }
            inline uint32 EndId() const
            {
                return (_end._id);
            }
            inline const string& Source() const
