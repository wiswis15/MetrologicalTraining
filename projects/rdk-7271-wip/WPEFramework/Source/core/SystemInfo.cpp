...
static string ConstructUniqueId(
    ...
    const uint8 DeviceIdLength,
    const TCHAR SystemPrefix[], 
    const uint8 SystemPrefixLength,
...
{
    static uint8* MACAddress = nullptr;
    static uint8 MACAddressBuffer[7];

    if (MACAddress == nullptr) {
        Core::AdapterIterator adapters;

        if (adapters.Next() == true) {
...
string SystemInfo::Id(const uint8 RawDeviceId[], const uint8 KeyLength)
{
    ...
    return (ConstructUniqueId(id.c_str(), static_cast<uint8>(id.size()),
    ...
    void SystemInfo::UpdateCpuStats() const
    {
        ...
        if(err == KERN_SUCCESS) {
            ...
            TRACE_L1("host_processor_info call failed", nullptr);
        }
        ...
        if (difftime(time(NULL), LastUpdateCpuStats) >= RefreshInterval) {
    ...
    void SystemInfo::UpdateTotalGpuRam()
    {
        ...
        SystemInfo::m_totalgpuram = NumberType<uint64>(TextFragment(response)).Value();
    ...
    void SystemInfo::UpdateFreeGpuRam()
    {
        ...
        // Convert string to integer.
        SystemInfo::m_freegpuram = NumberType<uint64>(TextFragment(response)).Value();
    ...
    void SystemInfo::UpdateRealtimeInfo()
    {
        ...
        mib[0] = CTL_HW;
        mib[1] = HW_MEMSIZE;
        sysctl(mib, 2, &m_totalram, &length, NULL, 0);

        mib[0] = CTL_KERN;
        mib[1] = KERN_BOOTTIME;
        length = sizeof(time);
        sysctl(mib, 2, &time, &length, NULL, 0);
        ...
        {
            TRACE_L1("vm_stat call failed", nullptr);
        }
    ...
    /* static */ bool SystemInfo::SetEnvironment(const string& name, const TCHAR* value)
    {
#ifdef __LINUX__
        if (value) {
    ...
    /* static */ bool SystemInfo::SetEnvironment(const string& name, const string& value)
    {
        return SetEnvironment(name, value.c_str());
    }
