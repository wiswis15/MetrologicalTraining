...
namespace Core {
    ...
    private:
        ...
        static const uint8 SLOT_ALLOCATION = 8;
    ...
    public:
        SerialMonitor()
            ...
            , m_Break(CreateEvent(NULL, TRUE, FALSE, NULL))
#endif
#ifdef __LINUX__
        ...
        {
            TRACE_L1("SerialPortMonitor: Closing [%d] ports", static_cast<uint32>(m_MonitoredPorts.size()));
            ...
            while ((index != m_MonitoredPorts.end()) && (fd_index < filledFileDescriptors)) {
                ...
                if (port == NULL) {
                    ...
                    if (fd_index < filledFileDescriptors) {
                        uint16 result = m_Slots[fd_index].revents;
...
#pragma GCC diagnostic pop
        ...
        uint32 Worker()
        {
            ...
            // Do we have enough space to allocate all file descriptors ?
            if ((m_MonitoredPorts.size() + 1) >= m_MaxSlots) {
                m_MaxSlots = ((((static_cast<uint32>(m_MonitoredPorts.size()) + 1) / SLOT_ALLOCATION) + 1) * SLOT_ALLOCATION);
            ...
            while (index != m_MonitoredPorts.end()) {
                ...
                if (port == NULL) {
                    ...
                    if ((::WaitForSingleObject(port->m_ReadInfo.hEvent, 0) == WAIT_OBJECT_0) && (::GetOverlappedResult(port->Descriptor(), &(port->m_ReadInfo), &info, FALSE))) {
                        ...
                        port->Read(static_cast<uint16>(info));
                    }
                    ...
                    if ((::WaitForSingleObject(port->m_WriteInfo.hEvent, 0) == WAIT_OBJECT_0) && (::GetOverlappedResult(port->Descriptor(), &(port->m_WriteInfo), &info, FALSE))) {
                        ...
                        port->Write(static_cast<uint16>(info));
                    }
...
// the virtuals might be called, which are destructed at this point !!!!
...
#endif
    {
        ...
        if ((receiveBuffer != 0) || (sendBuffer != 0)) {
            uint8* allocatedMemory = static_cast<uint8*>(::malloc(m_SendBufferSize + receiveBuffer));
            if (sendBuffer != 0) {
        ...
        const uint16 receiveBufferSize)
    {
        ...
        if (m_Descriptor == -1)
...
#endif
            {
                ...
                m_ReadInfo.hEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
                m_WriteInfo.hEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
#endif
                ...
                uint8* allocatedMemory = static_cast<uint8*>(::malloc(m_SendBufferSize + m_ReceiveBufferSize));
    ...
    uint32 SerialPort::Open(uint32 /* waitTime */)
    {
        uint32 result = 0;
    ...
    uint32 SerialPort::Close(uint32 /* waitTime */)
    {
        m_syncAdmin.Lock();
    ...
    void SerialPort::Trigger()
    {
        ...
        if ((m_State & (SerialPort::WRITE | SerialPort::OPEN | SerialPort::EXCEPTION)) == SerialPort::OPEN) {
#ifdef __WIN32__
            Write(0);
#endif // __WIN32__
#ifdef __LINUX__
...
#ifdef __WIN32__
    void SerialPort::Write(const uint16 writtenBytes)
    {
        m_syncAdmin.Lock();
        ...
        do {
            ...
            if (m_SendOffset < m_SendBytes) {
                ...
                if (::WriteFile(m_Descriptor,
                    ...
                    != FALSE) {
                    m_SendOffset += static_cast<uint16>(sendSize);
                }
                else {
    ...
    void SerialPort::Read(const uint16 readBytes)
    {
        m_syncAdmin.Lock();
        ...
        while ((m_State & (SerialPort::READ | SerialPort::EXCEPTION | SerialPort::OPEN)) == SerialPort::OPEN) {
            DWORD readBytes;
            uint32 l_Size = static_cast<uint32>(~0);
    ...
    void SerialPort::Write()
    {
        ...
        do {
            ...
            if (m_SendOffset < m_SendBytes) {
                ...
                if (sendSize != static_cast<uint32>(-1)) {
    ...
    void SerialPort::Read()
    {
        ...
        while ((m_State & (SerialPort::READ | SerialPort::EXCEPTION | SerialPort::OPEN)) == SerialPort::OPEN) {
            ...
            if ((l_Size != static_cast<uint32>(~0)) && (l_Size != 0)) {
                m_ReadBytes += l_Size;
