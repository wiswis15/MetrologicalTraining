...
namespace Core {
    ...
    private:
        ...
        public:
            ...
            inline TimedInfo(const uint64 time, const ACTIVECONTENT& contents)
                : m_ScheduleTime(time)
                , m_Info(contents)
            {
            }
            ...
            inline void ScheduleTime(const uint64 scheduleTime)
            {
        ...
        public:
            inline TimeWorker(TimerType& parent, const uint32 stackSize, const TCHAR* timerName)
    ...
    public:
        TimerType(const uint32 stackSize, const TCHAR* timerName)
            : m_PendingQueue()
        ...
        void Trigger(const uint64& time, const CONTENT& info)
        {
    ...
    protected:
        uint32 Process()
        {
            uint32 delayTime = Core::infinite;
            ...
            while ((m_PendingQueue.empty() == false) && (m_PendingQueue.front().ScheduleTime() <= now)) {
                ...
                uint64 reschedule = info.Content().Timed(info.ScheduleTime());
            ...
            if (m_PendingQueue.empty() == true) {
                ...
                if (delta >= m_PendingQueue.front().ScheduleTime()) {
                    ...
                    delayTime = static_cast<uint32>((m_NextTrigger - delta) / Time::TicksPerMillisecond);
                }
            }
    ...
    public:
        WatchDogType(const uint32 stackSize, const TCHAR* threadName)
        ...
        WatchDogType(const uint32 stackSize, const TCHAR* threadName, HANDLER source)
            : Thread(stackSize, threadName)
            , _job(source)
            , _delay(Core::infinite)
        {
        ...
        inline void Arm(uint32 waitTime)
        {
            Thread::Lock();
            _delay = waitTime;
            Run();
        ...
        inline void Arm(uint32 waitTime, HANDLER source)
        {
            Thread::Lock();
            _delay = waitTime;
            _job = source;
    ...
    private:
        virtual uint32 Worker()
        {
            uint32 nextDelay = Core::infinite;
