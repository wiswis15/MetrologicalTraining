...
namespace Core {
    ...
    public:
        ...
        uint16 Read(uint8* dataFrame, const uint16 maxSendSize) const
        {
            uint16 result = 0;
            ...
            if (_head != _tail) {
                ...
                if ((_tail < _head) && (result < maxSendSize)) {
                    uint16 copySize = (static_cast<uint16>(_head - _tail) < static_cast<uint16>(maxSendSize - result) ? (_head - _tail) : (maxSendSize - result));
        ...
        uint16 Write(const uint8* dataFrame, const uint16 receivedSize)
        {
            uint32 freeBuffer = Free();
            uint32 result = ((receivedSize + _head) > _size ? (_size - _head) : receivedSize);
            ...
            while (result < receivedSize) {
                ...
                uint32 copySize = ((receivedSize - result) > static_cast<uint16>(_size) ? _size : (receivedSize - result));
            ...
            if (freeBuffer < receivedSize) {
                ...
                _tail = ((_head + 1) < static_cast<uint16>(_size) ? (_head + 1) : 0);
            }
