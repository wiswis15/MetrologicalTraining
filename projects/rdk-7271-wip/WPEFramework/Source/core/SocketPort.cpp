...
namespace Core {
static uint32 SLEEPSLOT_TIME = 100;
...
private:
#ifdef __LINUX__
    static const uint8  FILE_DESCRIPTOR_ALLOCATION = 64;
    static const uint32 MAX_FILE_DESCRIPTORS = 3000;
#endif

    SocketMonitor(const SocketMonitor&) = delete;
    SocketMonitor& operator= (const SocketMonitor&) = delete;

    static constexpr uint32 WatchDogStackSize = 1024 * 1024; // Was 64K
    ...
    public:
        MonitorWorker(SocketMonitor& parent) :
            ...
            if (g_SocketInitializer.IsInitialized() == false) {
                TRACE_L1("SocketPortMonitor: Thread ID [%d]", (unsigned int) Id());
            }
#else
            TRACE_L1("SocketPortMonitor: Thread ID [%d]", Id());
#endif
        }
...
public:
    ...
    ~SocketMonitor() {
        TRACE_L1("SocketPortMonitor: Closing [%d] sockets", static_cast<uint32>(m_MonitoredPorts.size()));
...
public:
...
#ifdef __DEBUG__
    inline uint32 SocketsInState(const SocketPort::enumState state) const {
        switch (state) {
...
#ifdef SOCKET_TEST_VECTORS
    inline uint32 MonitorRuns () const {
    ...
    inline void Break() {
        ...
        m_ThreadInstance->Signal(SIGUSR1);
#endif
...
#pragma GCC diagnostic ignored "-Wunused-result"
    uint32  Worker() {
        ...
        while (index != m_MonitoredPorts.end()) {
            ...
            m_FDArray[filledFileDescriptors].events = POLLIN|POLLHUP|((port->m_State & SocketPort::WRITE) != 0 ? POLLOUT : 0);
        ...
        if (filledFileDescriptors > 1) {
...
#endif
                uint32 VARIABLE_IS_NOT_USED bytes = read(m_SignalFD, &info, sizeof(info));
            ...
            while ( fd_index  < filledFileDescriptors ) {
                ...
                uint16 flagsSet = m_FDArray[fd_index].revents;
...
#pragma GCC diagnostic pop
    ...
    uint32  Worker() {
        ...
        while (index != m_MonitoredPorts.end()) {
            ...
            ::WSAEnumNetworkEvents(socket->Socket(), NULL, &networkEvents);

            uint16 result = static_cast<uint16>(networkEvents.lNetworkEvents);
...
#ifdef SOCKET_TEST_VECTORS
    uint32			      m_MonitorRuns;
#endif

#ifdef __LINUX__
    uint32          m_MaxFileDescriptors;
...
//////////////////////////////////////////////////////////////////////
#ifdef __DEBUG__
/* static */ uint32 SocketPort::GetCallStack(void* buffers[], uint32 length) {
    return (::GetCallStack(g_SocketMonitor.Id(), buffers, length));
}
#endif

uint32 SocketPort::WaitForOpen (const uint32 time) const {

    // Make sure the state does not change in the mean time.
    m_syncAdmin.Lock();

    uint32 waiting = (time == Core::infinite ? Core::infinite : time); // Expect time in MS.
    ...
    while ( (waiting > 0) && (IsOpen() == false) ) {
        ...
        uint32 sleepSlot = (waiting > SLEEPSLOT_TIME ?  SLEEPSLOT_TIME : waiting);
    ...
    uint32 result = (((time == 0) || (IsOpen() == true)) ? Core::ERROR_NONE : Core::ERROR_TIMEDOUT);
...
uint32 SocketPort::WaitForClosure (const uint32 time) const {
    // If we build in release, we do not want to "hang" forever, forcefull close after 20S waiting...
#ifdef __DEBUG__
    uint32 waiting = time; // Expect time in MS
    uint32 reportSlot = 0;
#else
    uint32 waiting = (time == Core::infinite ? 20000 : time);
#endif
...
/* static */ uint32 SocketPort::SocketsInState(const enumState state) {
#ifdef __DEBUG__
...
#ifdef SOCKET_TEST_VECTORS
/* static */ uint32 SocketPort::MonitorRuns() {
...
SocketPort::SocketPort(
    ...
    const uint16       nSendBufferSize,
    const uint16       nReceiveBufferSize) :
...
uint32 SocketPort::TTL(const uint8 ttl)
{
...
void SocketPort::BufferAlignment (SOCKET socket) {
    ...
    int value;
    uint32 receiveBuffer = m_ReceiveBufferSize;
    uint32 sendBuffer = m_SendBufferSize;

    if (m_ReceiveBufferSize == static_cast<uint16>(~0)) {
        ::getsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char*) &value, &valueLength);

        receiveBuffer = static_cast<uint32>(value);
    ...
    if (m_SendBufferSize == static_cast<uint16>(~0)) {
        ::getsockopt(socket, SOL_SOCKET, SO_SNDBUF, (char*) &value, &valueLength);

        sendBuffer = static_cast<uint32>(value);
    ...
    if ( (receiveBuffer != 0) || (sendBuffer != 0) ) {
        uint8* allocatedMemory =  static_cast<uint8*> (::malloc(sendBuffer + receiveBuffer));
        if (sendBuffer != 0) {
...
SOCKET SocketPort::ConstructSocket(NodeId& localNode) {
    ...
    if ((l_Result = ::socket(localNode.Type(), SocketMode(), 0)) == INVALID_SOCKET) {
        TRACE_L1("Error on creating socket SOCKET. Error %d", __ERRORRESULT__);
    ...
    if (l_Result != INVALID_SOCKET) {
        // Do we need to find something to bind to or is it pre-destined
        if ( ((localNode.Type() == NodeId::TYPE_DOMAIN)  && (m_SocketType == SocketPort::LISTEN)) ||
                (((localNode.Type() == NodeId::TYPE_IPV4) || (localNode.Type() == NodeId::TYPE_IPV6)) && ((localNode.PortNumber() != 0) || (SocketMode() != SOCK_STREAM))) ) {
            if (::bind(l_Result, localNode, localNode.Size()) != SOCKET_ERROR) {
...
uint32 SocketPort::Open(const uint32 waitTime) {
    uint32 nStatus = Core::ERROR_ILLEGAL_STATE;
    ...
    if ((m_State & (SocketPort::LINK|SocketPort::OPEN|SocketPort::MONITOR)) == (SocketPort::LINK|SocketPort::OPEN)) {
        ...
        m_Socket = ConstructSocket(m_LocalNode);
        ...
        if(m_Socket != INVALID_SOCKET) {
            if( (m_SocketType == DATAGRAM) || ((m_SocketType == RAW) && (m_RemoteNode.IsValid() == false)) ) {
                ...
                if(::connect(m_Socket, m_RemoteNode, m_RemoteNode.Size()) != SOCKET_ERROR) {
                    m_State = SocketPort::LINK|SocketPort::OPEN|SocketPort::READ;
                    nStatus = Core::ERROR_NONE;
                } else {
...
uint32 SocketPort::Close(const uint32 waitTime) {
    ...
    if (m_Socket != INVALID_SOCKET) {
        ...
        if ((m_State != 0) && ((m_State & SHUTDOWN) == 0)) {
            ...
            if ( ((m_State & LINK) == 0) || ((m_State & OPEN) == 0) ) {
                // This is a conectionless link, do not expect a close from the otherside.
...
void SocketPort::Write() {
    ...
    while (((m_State & (SocketPort::WRITE|SocketPort::SHUTDOWN|SocketPort::OPEN|SocketPort::EXCEPTION)) == SocketPort::OPEN) && (dataLeftToSend == true)) {
        ...
        if (dataLeftToSend == true) {
            ...
            if (sendSize != static_cast<uint32>(SOCKET_ERROR)) {
                m_SendOffset += sendSize;
            } else {
                uint32 l_Result = __ERRORRESULT__;
...
void SocketPort::Read() {
    ...
    while ((m_State & (SocketPort::READ|SocketPort::EXCEPTION|SocketPort::OPEN)) == SocketPort::OPEN) {
        ...
        } else if (l_Size != static_cast<uint32>(SOCKET_ERROR)) {
            m_ReadBytes += l_Size;
        } else {
            uint32 l_Result = __ERRORRESULT__;
        ...
        if (m_ReadBytes != 0) {
            uint16 handledBytes = ReceiveData(m_ReceiveBuffer, m_ReadBytes);
