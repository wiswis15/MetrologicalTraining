...
namespace Core {
    ...
    public:
        ...
        inline uint8 IsTerminated(const TCHAR character) const
        {
            return (character == '\0' ? 0x80 | 0x00 : 0x00);
        }
        inline uint8 SizeOf() const
        {
            return (1);
        }
...
#pragma warning(default : 4355)
    ...
    private:
        ...
        uint16 SendData(uint8* dataFrame, const uint16 maxSendSize)
        {
            ...
            if (_offset == static_cast<uint32>(~0)) {
            ...
            if (_sendQueue.size() > 0) {
                ...
                if ((maxSendSize != result) && (_offset >= (sendObject.size() * sizeof(TCHAR))) && (_offset < ((sendObject.size() + (_terminator.SizeOf())) * sizeof(TCHAR)))) {
                    uint8 markerSize = (static_cast<uint8>(_terminator.SizeOf()) * sizeof(TCHAR));
                    uint8 markerOffset = ((sendObject.size() * sizeof(TCHAR)) - _offset);
                    uint16 size = ((markerSize - markerOffset) > (maxSendSize - result) ? (maxSendSize - result) : (markerSize - markerOffset));
        ...
        inline void Convert(const uint8* dataFrame, wchar_t& entry) const
        {
            entry = ntohs((dataFrame[0] << 8) | dataFrame[1]);
        }
        inline void Convert(const uint8* dataFrame, char& entry) const
        {
            entry = (dataFrame[0]);
        }
        uint16 ReceiveData(uint8* dataFrame, const uint16 receivedSize)
        {
            for (uint16 index = 0; index < receivedSize; index += sizeof(TCHAR)) {
                TCHAR character;
                ...
                // 0x40 -> Termination sequence in Progress.
                // 0x03 -> Number of characters to delete
                uint8 terminated = _terminator.IsTerminated(character);

                if ((terminated & 0x80) == 0) {
                    _receiver.push_back(character);
                }
                else {
                    uint8 dropCharacters = terminated & 0x03;
        ...
        inline uint16 SendCharacters(uint8* dataFrame, const TCHAR stream[], const uint8 delta, const uint16 total)
        {
