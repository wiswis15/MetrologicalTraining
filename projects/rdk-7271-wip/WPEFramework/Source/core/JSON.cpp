...
namespace Core {
    namespace JSON {
        ...
        uint16 IElement::Serializer::Serialize(uint8 stream[], const uint16 maxLength)
        {
            uint16 result = 0;
            ...
            if (_root != NULL) {
                ...
                while ((result < maxLength) && (_state != STATE_REPORT)) {
                    ...
                    case STATE_OPEN: {
                        bool container = (dynamic_cast<ILabelIterator*>(current) != NULL);
                    ...
                    case STATE_VALUE: {
                        if (current->Element()->Type() == PARSE_BUFFERED) {
                            ...
                            uint16 write = ((maxLength - result) > (static_cast<uint16>(_buffer.length()) - _offset) ? (static_cast<uint16>(_buffer.length()) - _offset) : (maxLength - result));
        ...
        uint16 IElement::Deserializer::Deserialize(const uint8 stream[], const uint16 maxLength)
        {
            uint16 result = 0;

            _adminLock.Lock();

            IIterator* current = NULL;
            ...
            while ((result < maxLength) && (_state != STATE_HANDLED)) {
                ...
                case STATE_OPEN: {
                    ...
                    int character = (elementList == NULL ? '{' : '[');
                    // See if we find an opening bracket in this part
                    while ((result < maxLength) && (stream[result] != character)) {
                        result++;
                    }
                ...
                case STATE_CLOSE: {
                    while ((result < maxLength) && (current != NULL) && (stream[result] != ',')) {
                        if ((stream[result] == '}') || (stream[result] == ']')) {
