...
namespace Core {
    ...
    private:
        typedef std::map<uint32, Core::ProxyType<CLIENT> > ClientMap;
    ...
    private:
        ...
        public:
            ...
            void Submit(const uint32 ID, PACKAGE package)
            {
                _lock.Lock();
            ...
            inline void Suspend(const uint32 ID)
            {
                _lock.Lock();
            ...
            virtual void Accept(SOCKET& newClient, const NodeId& remoteId)
            {
                ...
                if (client->Open(0) == ERROR_NONE) {
                    ...
                    _clients.insert(std::pair<uint32, ProxyType<HANDLECLIENT> >(_nextClient++, client));
        ...
        private:
            ...
            typedef hasId<HANDLECLIENT, void (HANDLECLIENT::*)(uint32)> TraitId;

            template <typename SUBJECT>
            inline typename Core::TypeTraits::enable_if<SocketHandler<SUBJECT>::TraitId::value, void>::type
            __Id(HANDLECLIENT& object, const uint32 id)
            {
                object.Id(id);
            }
            ...
            template <typename SUBJECT>
            inline typename Core::TypeTraits::enable_if<!SocketHandler<SUBJECT>::TraitId::value, void>::type
            __Id(HANDLECLIENT&, const uint32)
            {
            }
        ...
        private:
            ...
            std::map<uint32, ProxyType<HANDLECLIENT> > _clients;
    ...
    public:
        ...
        {
            uint32 result = _handler.Close(waitTime);
        ...
        template <typename PACKAGE>
        inline void Submit(const uint32 ID, PACKAGE package)
        {
