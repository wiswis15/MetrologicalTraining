...
namespace Core {
    namespace System {
        ...
        extern "C" EXTERNAL uint32 Reboot();
    }
    ...
    private:
        ...
        static const uint32 RefreshInterval = 1;

    public:
        string Id(const uint8 RawDeviceId[], const uint8 KeyLength);

        // First byte of the RawDeviceId is the length of the DeviceId to follow.
        const uint8 * RawDeviceId() const;

        static bool GetEnvironment(const string& name, string& value);
        static bool SetEnvironment(const string& name, const TCHAR * value);
        static bool SetEnvironment(const string& name, const string& value);
        ...
        inline uint64 GetTotalRam() const
        {
            return m_totalram;
        }
        ...
        inline uint64 GetTotalGpuRam()
        {
            UpdateTotalGpuRam();
            return m_totalgpuram;
        }

        inline uint64 GetFreeGpuRam()
        {
            UpdateFreeGpuRam();
            return m_freegpuram;
        }
...
#define HAVE_TICK_COUNTER
        ...
        inline uint64 Ticks() const
        {
            ...
            return (static_cast<uint64>(mytime.tv_nsec) + (static_cast<uint64>(mytime.tv_sec) * 1000000000UL));
        }
...
#error "Come up with an implementation of a High Resolution tick counter for the compiler you are using."
    ...
    private:
        ...
        uint64 m_totalram;
        mutable uint32 m_uptime;
        mutable uint64 m_freeram;
        mutable double m_cpuload;
        mutable time_t LastUpdateCpuStats;
        mutable uint64 m_totalgpuram;
        mutable uint64 m_freegpuram;
