...
namespace Core {
	...
	public: // Methods
        ...
        uint32 Lock(unsigned int nSeconds);
        void Unlock();
        bool Locked() const;
	...
	public: // Methods
        ...
        // Time in milliseconds!
        uint32 Lock(unsigned int nSeconds);
        uint32 Unlock(unsigned int nCount = 1);
        uint32 TryUnlock(unsigned int nSeconds);
	...
	public: // Methods
        ...
        // Time in milliseconds!
        uint32 Lock(unsigned int nTime);
        void ResetEvent();
        void SetEvent();
        void PulseEvent();
        bool IsSet() const;
	...
	public:
		void Ring();
		uint32 Wait(const uint32 waitTime) const;
		void Acknowledge();
    ...
    EXTERNAL uint32 InterlockedIncrement(volatile int& a_Number);
    EXTERNAL uint32 InterlockedDecrement(volatile int& a_Number);
}
} // namespace Core
