...
namespace Core {
    ...
    CyclicBuffer::CyclicBuffer(const string& fileName, const uint32 bufferSize, const bool overwrite)
        : _buffer(
              fileName,
    ...
    // This is in MS...
    uint32 CyclicBuffer::SignalLock(const uint32 waitTime)
    {

        uint32 result = waitTime;
    ...
    uint32 CyclicBuffer::Read(uint8 buffer[], const uint32 length)
    {
        ...
        uint32 result;
        while (!foundData) {
            uint32 oldTail = _administration->_tail;
            uint32 head = _administration->_head;
            uint32 offset = oldTail & _administration->_tailIndexMask;
            ...
            uint32 roundCount = oldTail / (1 + _administration->_tailIndexMask);
            if ((offset + result) < _maxSize) {
                memcpy(buffer, _realBuffer + offset, result);

                uint32 newTail = offset + result + roundCount * (1 + _administration->_tailIndexMask);
                if(!std::atomic_compare_exchange_weak(&(_administration->_tail), &oldTail, newTail)) {
                	foundData = false;
                }
            } else {
                uint32 part1(_maxSize - offset);
                uint32 part2(result - part1);
                ...
                uint32 newTail = part2 + roundCount * (1 + _administration->_tailIndexMask);
                if (!std::atomic_compare_exchange_weak(&(_administration->_tail), &oldTail, newTail)) {
                	foundData = false;
                }
            }
    ...
    uint32 CyclicBuffer::Write(const uint8 buffer[], const uint32 length)
    {
    	ASSERT(length < _maxSize);

    	uint32 head = _administration->_head;
    	bool startingEmpty = (Used() == 0);
    	uint32 writeStart = head;
    	...
    	if (_administration->_reservedPID != 0) {
    		...
    		uint32 newReservedWritten = _administration->_reservedWritten + length;
    	...
    	// Perform actual copy.
    	uint32 writeEnd = (writeStart + length) % _maxSize;
    	if (writeEnd > writeStart) {
    		// Easy case: one pass.
            memcpy(_realBuffer + writeStart, buffer, length);
    	} else {
    		// Copying beyond end of buffer, do in two passes.
			uint32 firstLength = _maxSize - writeStart;
			uint32 secondLength = length - firstLength;
    ...
    void CyclicBuffer::AssureFreeSpace(uint32 required)
    {
    	uint32 oldTail = _administration->_tail;
    	uint32 head = _administration->_head;

    	uint32 tail = oldTail & _administration->_tailIndexMask;
    	uint32 free = Free(head, tail);

    	while (free < required) {
    	    uint32 remaining = required - free;
    		Cursor cursor(*this, oldTail, remaining);
        	uint32 offset = GetOverwriteSize(cursor);
        	ASSERT((offset + free) >= required);

    		uint32 newTail = cursor.GetCompleteTail(offset);
    ...
    uint32 CyclicBuffer::Reserve(const uint32 length)
    {
#ifdef __WIN32__
    	...
    	bool noOtherReservation = atomic_compare_exchange_weak(&(_administration->_reservedPID), &expectedProcessId, processId);
    ...
    uint32 CyclicBuffer::Lock(const bool dataPresent, const uint32 waitTime)
    {
        ...
        do {
            ...
            if ((((_administration->_state.load()) & state::LOCKED) != state::LOCKED) && ((dataPresent == false) || (Used() > 0))) {
                std::atomic_fetch_or(&(_administration->_state), static_cast<uint16>(state::LOCKED));
    ...
    uint32 CyclicBuffer::Unlock()
    {
        ...
        if (_administration->_lockPID == Core::ProcessInfo().Id()) {
            ...
            _administration->_lockPID = 0;
            std::atomic_fetch_and(&(_administration->_state), static_cast<uint16>(~state::LOCKED));
    ...
    uint32 CyclicBuffer::Peek(uint8 buffer[], const uint32 length) const
    {
        ASSERT(length <= _maxSize);
        ...
        while (!foundData) {
            ...
            if (readEnd < _maxSize) {
    			...
    			uint32 firstLength = _maxSize - tail;
    			uint32 secondLength = length - firstLength;
	...
	/* virtual */ uint32 CyclicBuffer::GetOverwriteSize(Cursor& cursor)
	{
		// Easy case: just return requested bytes.
		return cursor.Size();
	}

	/* virtual */ uint32 CyclicBuffer::GetReadSize(Cursor& cursor)
	{
		// Easy case: just return requested bytes.
		return cursor.Size();
	}
