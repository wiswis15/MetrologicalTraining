...
namespace Core {
    struct IIterator {
        ...
        virtual void Reset(const uint32 position) = 0;
    ...
    public:
        ...
        virtual void Reset(const uint32 position)
        {
            if (m_Container != NULL) {
                if (position == 0) {
                    ...
                    // Move forward the requested number of steps..
                    for (uint32 teller = 1; teller < position; teller++) {
                        m_Iterator++;
                    }
        ...
        virtual bool Next()
        {
            if ((m_Container != NULL) && (m_Index != Count() + 1)) {
                m_Index++;
        ...
        {
            return (m_Container == NULL ? 0 : static_cast<uint32>(m_Container->size()));
        }
    ...
    public:
        ...
        {
            if (IteratorType<CONTAINER, ELEMENT>::Container() != NULL) {
                IteratorType<CONTAINER, ELEMENT>::Container()->ReadLock();
            }
        }
        ~LockableIteratorType()
        {
            if (IteratorType<CONTAINER, ELEMENT, ITERATOR>::Container() != NULL) {
