...
namespace Core {
    ...
    public:
        ...
        {
            uint32 filenameOffset = FileNameOffset(name);
            uint32 offset = ExtensionOffset(name);
        ...
        {
            string result;
            uint32 offset = FileNameOffset(name);
...
#endif
#ifdef __POSIX__
            return (static_cast<uint32>(errno));
#endif
        }
        ...
        bool SetSize(const uint64 size)
        {
            bool result;
#ifdef __WIN32__
...
#endif
#ifdef __WIN32__
            _handle = ::CreateFile(_name.c_str(), (GENERIC_READ | GENERIC_WRITE), (_sharable ? (FILE_SHARE_READ | FILE_SHARE_WRITE) : 0), NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
            LoadFileInfo();
            return (IsOpen());
        }
...
#endif
#ifdef __WIN32__
            _handle = ::CreateFile(_name.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
...
#endif
#ifdef __WIN32__
            _handle = ::CreateFile(_name.c_str(), (readOnly ? GENERIC_READ : GENERIC_READ | GENERIC_WRITE), (_sharable ? (FILE_SHARE_READ | (readOnly ? 0 : FILE_SHARE_WRITE)) : 0), NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
            LoadFileInfo();
            return (IsOpen());
        }
...
#endif
#ifdef __WIN32__
            _handle = ::CreateFile(_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            if (_handle != INVALID_HANDLE_VALUE) {
        ...
        uint32 Write(const uint8 buffer[], uint32 size)
        {
            ...
            ::WriteFile(_handle, buffer, size, &writtenBytes, NULL);
            return static_cast<uint32>(writtenBytes);
#endif
        }
        uint32 Read(uint8 buffer[], uint32 size) const
        {
            // Only call these methods if the file is open.
            ASSERT(IsOpen());
...
#ifdef __WIN32__
            DWORD readBytes = 0;
            ::ReadFile(_handle, buffer, size, &readBytes, NULL);
            return static_cast<uint32>(readBytes);
#endif
        }

        bool Position(const bool relative, sint32 offset)
        {
            // Only call these methods if the file is open.
            ASSERT(IsOpen());
...
#endif
#ifdef __WIN32__
            return (::SetFilePointer(_handle, offset, NULL, (relative ? FILE_CURRENT : FILE_BEGIN)) != INVALID_SET_FILE_POINTER);
#endif
        }
...
#endif
#ifdef __WIN32__
                DWORD newPos = ::SetFilePointer(_handle, 0, NULL, FILE_CURRENT);

                if (newPos != INVALID_SET_FILE_POINTER) {
                    ASSERT(newPos >= 0);

                    result = (static_cast<sint32>(newPos));
                }
#endif
            }
    ...
    private:
        inline static uint32 ExtensionOffset(const string& name)
        {
            int offset;
            return (((offset = name.find_last_of('.')) == -1) ? 0 : offset + 1);
        }
        inline static uint32 FileNameOffset(const string& name)
        {
            int offset;
    ...
    public:
        ...
        bool IsValid() const
        {
            return ((_dirFD != NULL) && (_entry != NULL));
        }
        void Reset()
        {
            if (_dirFD != NULL) {
        ...
        bool Next()
        {
            ...
            if ((_dirFD == NULL) && (_entry == NULL)) {
                _dirFD = opendir(_name.c_str());
            }

            while ((_dirFD != NULL) && (valid == false)) {
