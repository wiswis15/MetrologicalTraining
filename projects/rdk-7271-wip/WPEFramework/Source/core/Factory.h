...
namespace Core {
    ...
    private:
        ...
        struct IFactory {
            ...
            virtual uint32 CreatedElements() const = 0;
            virtual uint32 QueuedElements() const = 0;
            virtual uint32 CurrentQueueSize() const = 0;
        };
        ...
        public:
            InternalFactoryType(const uint32 initialQueueSize)
        ...
        public:
            ...
            virtual uint32 CreatedElements() const
            {
                return (_warehouse.CreatedElements());
            }
            virtual uint32 QueuedElements() const
            {
                return (_warehouse.QueuedElements());
            }
            virtual uint32 CurrentQueueSize() const
            {
    ...
    public:
        ...
        FactoryType(const uint8 /* queueuSize */)
        {
        }
        ~FactoryType()
        {
    ...
    public:
        ...
        template <typename ACTUALELEMENT>
        void CreateFactory(const uint32 initialQueueSize)
        {
        ...
        template <typename ACTUALELEMENT>
        void CreateFactory(const IDENTIFIER& identifier, const uint32 initialQueueSize)
        {
            typename std::map<IDENTIFIER, IFactory*>::iterator index(_receptors.find(identifier));
