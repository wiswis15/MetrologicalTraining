...
namespace Core {
    ...
    public:
        ...
        public:
            ...
            uint16 Serialize(uint8 stream[], const uint16 maxLength)
            {
                ...
                while ((_current != NULL) && (result < maxLength)) {
                    if (_offset < 4) {
                        uint32 length = _length + CommandSize();

                        // Write the length. Continue as long as the top bt is active..
                        while ((_offset < 4) && (result < maxLength)) {
                            uint32 value = length >> (7 * _offset);
                    ...
                    while ((_offset < 8) && (result < maxLength)) {
                        uint32 value = _current->Label() >> (7 * (_offset - 4));
                        stream[result] = ((value & 0x7F) | (value >= 0x80 ? 0x80 : 0x00));
                        result++;
                    ...
                    if (result < maxLength) {
                        // Write the command, Same structure as length..
                        uint16 handled = _current->Serialize(&stream[result], maxLength - result, _offset - 8);
        ...
        private:
            inline uint32 CommandSize() const
            {
        ...
        public:
            ...
            virtual IMessage* Element(const uint32& label) = 0;

            uint16 Deserialize(const uint8 stream[], const uint16 maxLength)
            {
                uint16 result = 0;

                while (result < maxLength) {
                    if ((_current == NULL) && (_offset < 8)) {
                    ...
                    if ((_offset - 8) < _length) {
						...
						uint16 handled ((maxLength - result) > static_cast<uint16>(_length - (_offset - 8)) ? static_cast<uint16>(_length - (_offset - 8)) : (maxLength - result));
    ...
    public:
        ...
        virtual uint32 Label() const = 0;
        virtual uint32 Length() const = 0;
        virtual uint16 Serialize(uint8[] /* stream*/, const uint16 /* maxLength */, const uint32 offset) const = 0;
        virtual uint16 Deserialize(const uint8[] /* stream*/, const uint16 /* maxLength */, const uint32 offset) = 0;
    };
    ...
    namespace IPC {
        ...
        public:
            ...
            inline uint16 Serialize(uint8 buffer[], const uint16 length, const uint32 offset) const
            {
                uint16 size = ((Length() - offset) > length ? length : (Length() - offset));
                ::memcpy(buffer, &(reinterpret_cast<const uint8*>(_data.c_str())[offset]), size);

                return (size);
            }
            uint16 Deserialize(const uint8 buffer[], const uint16 length, const uint32 /* offset */)
            {
                uint16 teller = 0;
        ...
        public:
            ...
            explicit inline Text(const TCHAR text[], const uint16 length = 0)
            ...
            {
                _length = static_cast<uint16>(RHS.length());
        ...
        public:
            ...
            inline uint16 Deserialize(const uint8 stream[], const uint16 maxLength, const uint32 offset)
            {
                ...
                if (offset < (LENGTH * sizeof(TCHAR))) {
                    ...
                    ::memcpy(&(reinterpret_cast<uint8*>(&_text)[offset]), stream, result);
    ...
    template <const uint32 IDENTIFIER, typename PARAMETERS, typename RESPONSE>
    class IPCMessageType : public IIPC {
    private:
        template <typename PACKAGE, const uint32 REALIDENTIFIER>
        ...
        public:
            ...
            virtual uint16 Serialize(uint8 stream[], const uint16 maxLength, const uint32 offset) const
            {
                return (_Serialize<PACKAGE, REALIDENTIFIER>(stream, maxLength, offset));
            }
            virtual uint16 Deserialize(const uint8 stream[], const uint16 maxLength, const uint32 offset)
            {
        ...
        private:
            ...
            template <typename SUBJECT, const uint32 ID>
            ...
            typedef hasLength<PACKAGE, uint32 (PACKAGE::*)() const> TraitLength;

            template <typename SUBJECT, const uint32 ID>
            inline typename Core::TypeTraits::enable_if<RawSerializedType<SUBJECT, ID>::TraitLength::value, uint32>::type
            _Length() const
            {
            ...
            typedef hasSerialize<PACKAGE, uint16 (PACKAGE::*)(uint8[], const uint16, const uint32) const> TraitSerialize;

            template <typename SUBJECT, const uint32 ID>
            inline typename Core::TypeTraits::enable_if<RawSerializedType<SUBJECT, ID>::TraitSerialize::value, uint16>::type
            _Serialize(uint8 stream[], const uint16 maxLength, const uint32 offset) const
            {
            ...
            {
                uint16 result = 0;
                uint32 packageLength = _Length<SUBJECT, ID>();

                if (offset < packageLength) {
                    result = ((maxLength > (packageLength - offset)) ? (packageLength - offset) : maxLength);
                    ::memcpy(stream, &(reinterpret_cast<const uint8*>(&_package)[offset]), result);
                }
                return (result);
            }
            ...
            typedef hasDeserialize<PACKAGE, uint16 (PACKAGE::*)(const uint8[], const uint16, const uint32)> TraitDeserialize;

            template <typename SUBJECT, const uint32 ID>
            inline typename Core::TypeTraits::enable_if<RawSerializedType<SUBJECT, ID>::TraitDeserialize::value, uint16>::type
            _Deserialize(const uint8 stream[], const uint16 maxLength, const uint32 offset)
            {
            ...
            _Deserialize(const uint8 stream[], const uint16 maxLength, const uint32 offset)
            {
                ...
                if (offset < sizeof(SUBJECT)) {
                    ...
                    ::memcpy(&(reinterpret_cast<uint8*>(&_package)[offset]), stream, result);
                }
                return (result);
            }
    ...
    public:
        ...
        private:
            ...
            inline void Factory(Core::ProxyType<FactoryType<IIPC, uint32> >& factory)
            {
        ...
        public:
            IPCFactory(Core::ProxyType<FactoryType<IIPC, uint32> >& factory)
                : _lock()
                , _inbound()
                , _outbound()
                , _callback(nullptr)
            ...
            ~IPCFactory()
            {
                ...
                std::map<uint32, ProxyType<IIPCServer> >::iterator index(_handlers.begin());
        ...
        public:
            inline void Register(const ProxyType<IIPCServer>& handler)
            {
                ...
                _handlers.insert(std::pair<uint32, ProxyType<IIPCServer> >(handler->Id(), handler));
            ...
            inline void Unregister(const ProxyType<IIPCServer>& handler)
            {
                ...
                std::map<uint32, ProxyType<IIPCServer> >::iterator index(_handlers.find(handler->Id()));
            ...
            inline ProxyType<IMessage> Element(const uint32& identifier)
            {
                ProxyType<IMessage> result;
                uint32 searchIdentifier(identifier >> 1);
                ...
                if (identifier & 0x01) {
                    if ((_outbound.IsValid() == true) && (_outbound->Label() == searchIdentifier)) {
                        ...
                        TRACE_L1("Unexpeced response message for ID [%d].\n", searchIdentifier);
                    }
                }
                else {
            ...
            inline ProxyType<IIPCServer> ReceivedMessage(const Core::ProxyType<IMessage>& rhs, Core::ProxyType<IIPC>& inbound)
            {
                ...
                // If this is *NOT* the outbound call, it is inbound and thus it must have been registered
                    ...
                    std::map<uint32, ProxyType<IIPCServer> >::iterator index(_handlers.find(_inbound->Label()));
        ...
        private:
            ...
            Core::ProxyType<FactoryType<IIPC, uint32> > _factory;
            std::map<uint32, ProxyType<IIPCServer> > _handlers;
        };
    ...
    public:
        IPCChannel(Core::ProxyType<FactoryType<IIPC, uint32> >& factory)
    ...
    public:
        ...
        inline uint32 Invoke(ProxyType<ACTUALELEMENT>& command, IDispatchType<IIPC>* completed)
        {
            Core::ProxyType<IIPC> base(Core::proxy_cast<IIPC>(command));
            return (Execute(base, completed));
        }
        template <typename ACTUALELEMENT>
        inline uint32 Invoke(ProxyType<ACTUALELEMENT>& command, const uint32 waitTime)
        {
            Core::ProxyType<IIPC> base(Core::proxy_cast<IIPC>(command));
            return (Execute(base, waitTime));
        }
        inline uint32 Invoke(ProxyType<Core::IIPC>& command, IDispatchType<IIPC>* completed)
        {
            return (Execute(command, completed));
        }
        inline uint32 Invoke(ProxyType<Core::IIPC>& command, const uint32 waitTime)
        {
            return (Execute(command, waitTime));
        }

        virtual uint32 ReportResponse(Core::ProxyType<IIPC>& inbound) = 0;

    private:
        virtual uint32 Execute(ProxyType<IIPC>& command, IDispatchType<IIPC>* completed) = 0;
        virtual uint32 Execute(ProxyType<IIPC>& command, const uint32 waitTime) = 0;
    ...
    private:
        ...
        public:
            uint32 Wait(const uint32 waitTime)
            {
    ...
    public:
        ...
        template <typename ARG1>
        IPCChannelType(Core::ProxyType<FactoryType<IIPC, uint32> >& factory, ARG1 arg1)
            : IPCChannel(factory)
            , _link(this, &_administration, arg1)
            , _extension(this)
        {
        ...
        template <typename ARG1, typename ARG2>
        IPCChannelType(Core::ProxyType<FactoryType<IIPC, uint32> >& factory, ARG1 arg1, ARG2 arg2)
            : IPCChannel(factory)
            , _link(this, &_administration, arg1, arg2)
            , _extension(this)
        {
        ...
        template <typename ARG1, typename ARG2, typename ARG3>
        IPCChannelType(Core::ProxyType<FactoryType<IIPC, uint32> >& factory, ARG1 arg1, ARG2 arg2, ARG3 arg3)
            : IPCChannel(factory)
            , _link(this, &_administration, arg1, arg2, arg3)
            , _extension(this)
        {
        ...
        template <typename ARG1, typename ARG2, typename ARG3, typename ARG4>
        IPCChannelType(Core::ProxyType<FactoryType<IIPC, uint32> >& factory, ARG1 arg1, ARG2 arg2, ARG3 arg3, ARG4 arg4)
            : IPCChannel(factory)
            , _link(this, &_administration, arg1, arg2, arg3, arg4)
            , _extension(this)
        {
        ...
        template <typename ARG1, typename ARG2, typename ARG3, typename ARG4, typename ARG5>
        IPCChannelType(Core::ProxyType<FactoryType<IIPC, uint32> >& factory, ARG1 arg1, ARG2 arg2, ARG3 arg3, ARG4 arg4, ARG5 arg5)
            : IPCChannel(factory)
            , _link(this, &_administration, arg1, arg2, arg3, arg4, arg5)
            , _extension(this)
        {
    ...
    public:
        ...
        virtual uint32 ReportResponse(Core::ProxyType<IIPC>& inbound)
        {
    ...
    private:
        ...
        virtual uint32 Execute(ProxyType<IIPC>& command, IDispatchType<IIPC>* completed)
        {
            uint32 success = Core::ERROR_UNAVAILABLE;
        ...
        virtual uint32 Execute(ProxyType<IIPC>& command, const uint32 waitTime)
        {
            uint32 success = Core::ERROR_CONNECTION_CLOSED;
