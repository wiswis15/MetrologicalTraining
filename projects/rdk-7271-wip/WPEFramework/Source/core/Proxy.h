...
namespace Core {
	...
	public:
				...
				unsigned int AdditionalSize)
		{
			...
			if (AdditionalSize != 0) {
				Space = reinterpret_cast<uint8*>(::malloc(alignedSize + sizeof(uint32) + AdditionalSize));

				if (Space != NULL) {
					*(reinterpret_cast<uint32*>(&Space[alignedSize])) = AdditionalSize;
				}
			}
			else {
				...
				Space = reinterpret_cast<uint8*>(::malloc(alignedSize));
			}
	...
	public:
		...
		inline uint32 Release() const
		{
			...
			uint32 result = m_RefCount->Release();
	...
	public:
		inline uint8* Buffer()
		{
			return (&(AllocatedBuffer()[sizeof(uint32)]));
		}
		...
		{
			return (*reinterpret_cast<const uint32*>(AllocatedBuffer()));
		}
		inline Core::ProxyType<ProxyElement> Clone(const uint32 size)
		{
		...
		inline uint8& operator[](const uint32 index)
		{
			...
			return (AllocatedBuffer()[sizeof(uint32) + index]);
		}
	...
	private:
		inline ProxyType<ProxyElement> InternalClone(const uint32 size, const TemplateIntToType<true>&) {
			ProxyObject<ProxyElement>* newElement(new (size) ProxyObject<ProxyElement>(static_cast<const CONTEXT&>(*this)));
			const uint32 copySize(size >= Size() ? static_cast<uint32>(Size()) : size);
			if (copySize > 0) {
				::memcpy(newElement->Buffer(), AllocatedBuffer(), copySize);
			}
			return (ProxyType<ProxyElement>(newElement, newElement));
		}
		inline Core::ProxyType<ProxyElement> InternalClone(const uint32 size, const TemplateIntToType<false>&) {
			return (Core::ProxyType<ProxyElement>());
		}
		inline uint8* AllocatedBuffer()
		{
			return (&(reinterpret_cast<uint8*>(this)[((sizeof(ProxyObject<ProxyElement>) + 3) & ((~0) ^ 0x3))]));
		}
		inline const uint8* AllocatedBuffer() const
		{
			return (&(reinterpret_cast<const uint8*>(this)[((sizeof(ProxyObject<ProxyElement>) + 3) & ((~0) ^ 0x3))]));
		}
	};
    ...
    public:
        ...
        inline uint32 CurrentQueueSize() const
        {
            return (m_Max);
        }
    ...
    public:
        ...
        Insert(
            ProxyType<CONTEXT>& a_Entry,
            uint32 a_WaitTime = Core::infinite);
    ...
    public:
        ProxyPoolType(const uint32 initialQueueSize)
            : _createdElements(0)
    ...
    public:
        ...
        inline uint32 CreatedElements() const
        {
            return (_createdElements);
        }
        inline uint32 QueuedElements() const
        {
            return (_queue.Count());
        }
        inline uint32 CurrentQueueSize() const
    ...
    private:
        ...
        public:
            virtual uint32 Release() const
            {
                uint32 result = Core::InterlockedDecrement(Core::ProxyObject<ELEMENT>::m_RefCount);
