...
namespace Core {
    ...
    static constexpr uint32 NTPToUNIXSeconds = (DayUNIXEpochStarts - DayNTPStarts) * SecondsPerDay;

    static uint8 MonthFromString(const TCHAR entry[])
    {
        assert(nullptr != entry);
        // Case sensitive quick compare :-)
        uint32 value = (static_cast<uint8>(entry[0]) << 16) | (static_cast<uint8>(entry[1]) << 8) | (static_cast<uint8>(entry[2]) << 0);
    ...
    static uint32 YearFromString(const TCHAR entry[], const uint8 maxLength, uint16& year)
    {
        assert(nullptr != entry);
        ...
        if (((index + 1) < maxLength) && (::isdigit(entry[index]) != 0) && (::isdigit(entry[index + 1]) != 0)) {
            ...
            if (((index + 3) < maxLength) && (::isdigit(entry[index + 2]) != 0) && (::isdigit(entry[index + 3]) != 0)) {
                year = static_cast<uint16>(((entry[index] - '0') * 1000) + ((entry[index + 1] - '0') * 100) + ((entry[index + 2] - '0') * 10) + (entry[index + 3] - '0'));
                index += 4;
            }
            else {
                year = static_cast<uint16>((entry[index] - '0') * 10) + (entry[index + 1] - '0');
    ...
    static uint32 TimeFromString(const TCHAR entry[], const uint8 maxLength, uint8& hours, uint8& minutes, uint8& seconds)
    {
        assert(nullptr != entry);
        ...
        if ((index + 5) < maxLength) {
            hours = static_cast<uint8>(entry[index] - '0');
            if (::isdigit(entry[index + 1]) != 0) {
                // hh:mm:ss
                hours = static_cast<uint8>((hours * 10) + (entry[index + 1] - '0'));
                index += 2;
            }
            else {
                // h:mm:ss
            ...
            if ((entry[index] == ':') && (::isdigit(entry[index + 1]) != 0)) {
                minutes = static_cast<uint8>(entry[index + 1] - '0');
                if (::isdigit(entry[index + 2]) != 0) {
                    // :mm:ss
                    minutes = static_cast<uint8>((minutes * 10) + (entry[index + 2] - '0'));
                    index += 3;
                }
                else {
                    // :m:ss ?
                ...
                if (entry[index] == ':') {
                    if (((index + 1) < maxLength) && (::isdigit(entry[index + 1]) != 0)) {
                        seconds = static_cast<uint8>(entry[index + 1] - '0');
                        if (((index + 2) < maxLength) && (::isdigit(entry[index + 2]) != 0)) {
                            // :ss
                            seconds = static_cast<uint8>((seconds * 10) + (entry[index + 2] - '0'));
                            index += 3;
                        }
                        else {
                            // :s
    ...
    bool Time::FromANSI(const string& buffer, const bool localTime)
    {
        ...
        uint32 index = 4;
        uint16 year = static_cast<uint16>(~0);
        uint8 month = static_cast<uint8>(~0);
        uint8 day = static_cast<uint8>(~0);
        uint8 hours = static_cast<uint8>(~0);
        uint8 minutes = static_cast<uint8>(~0);
        uint8 seconds = static_cast<uint8>(~0);
        ...
        if ((index + 3) < static_cast<uint32>(buffer.size())) {
            month = MonthFromString(&(buffer.c_str()[index]));

            if (month != static_cast<uint8>(~0)) {
                index += 3;

                // Skip Space
                while ((index < static_cast<uint32>(buffer.size())) && (::isdigit(buffer[index]) == 0)) {
                    index++;
                }

                if ((index + 7) < buffer.size()) {
                    // Now we should be on the day
                    day = static_cast<uint8>(buffer[index] - '0');
                    if (::isdigit(buffer[index + 1]) != 0) {
                        day = static_cast<uint8>((day * 10) + (buffer[index + 1] - '0'));
                        index += 2;
                    }
                    else {
                        index++;
                    ...
                    index += TimeFromString(&(buffer.c_str()[index]), static_cast<uint8>(buffer.size() - index), hours,
                        minutes, seconds);

                    index += YearFromString(&(buffer.c_str()[index]), static_cast<uint8>(buffer.size() - index), year);

                    if ((day != 0) && (seconds != static_cast<uint8>(~0)) && (year != static_cast<uint8>(~0))) {
    ...
    bool Time::FromRFC1123(const string& buffer)
    {
        ...
        if ((index + 14) < static_cast<uint32>(buffer.size())) {
            // Now we should be on the day
            day = static_cast<uint8>(buffer[index] - '0');
            if (::isdigit(buffer[index + 1]) != 0) {
                // dd
            ...
            if ((index + 11) < buffer.size()) {
                ...
                if (month != static_cast<uint8>(~0)) {
                    ...
                    if ((index + 7) < buffer.size()) {
                        index += YearFromString(&(buffer.c_str()[index]), static_cast<uint8>(buffer.size() - index),
                            year);

                        index += TimeFromString(&(buffer.c_str()[index]), static_cast<uint8>(buffer.size() - index),
                        ...
                        if ((day != 0) && (seconds != static_cast<uint8>(~0)) && (year != static_cast<uint8>(~0))) {
                            ...
                            while ((index < static_cast<uint32>(buffer.size())) && (::isspace(buffer[index]) != 0)) {
                                index++;
                            }

                            if ((index + 2) < static_cast<uint32>(buffer.size())) {
                                uint32 value = (static_cast<uint8>(buffer[index + 0]) << 16) | (static_cast<uint8>(buffer[index + 1]) << 8) | (static_cast<uint8>(buffer[index + 2]) << 0);
    ...
    bool Time::FromRFC1036(const string& buffer)
    {
        ...
        if ((index + 14) < static_cast<uint32>(buffer.size())) {
            ...
            if ((buffer[index]) == '-') {
                ...
                if ((month != static_cast<uint8>(~0)) && ((buffer[index + 3]) == '-')) {
                    index += 4;
    ...
    static uint64 OffsetTicksForEpoch = 11644473600000000ULL;
    ...
    // From the given system time to 1st of January 1970, 00:00:00
    static uint64 OffsetTicksForEpoch = 0ULL;
...
// Invariant for both Linux and Windows: internal time stored is always according to local time specification, so GMT / UTC if local time  is false, local time otherwise.
    ...
    Time::Time(const uint16 year, const uint8 month, const uint8 day, const uint8 hour, const uint8 minute, const uint8 second, const uint16 millisecond, const bool localTime)
    ...
    Time::Time(const uint64 time, bool localTime)
        : _time()
        , _isLocalTime(localTime)
    {
        FILETIME fileTime;
    ...
    uint64 Time::Ticks() const
    {
        ...
        uint64 microSeconds = (result.QuadPart / 10);
    ...
    uint8 Time::DayOfWeek() const
    {
        ...
        return (static_cast<uint8>(_time.wDayOfWeek));
    }

    uint16 Time::DayOfYear() const
    {
        struct tm convertedTime;
        convertedTime.tm_year = _time.wYear - 1900;
        convertedTime.tm_mon = _time.wMonth - 1;
        ...
        return (static_cast<uint16>(convertedTime.tm_yday));
    }
...
#pragma warning(default : 4996)
    ...
    Time::Time(const struct timespec& time, bool localTime)
    {
        ...
        // Calculate ticks..
        _ticks = (static_cast<uint64>(time.tv_sec) * MicroSecondsPerSecond) + (time.tv_nsec / NanoSecondsPerMicroSecond) + OffsetTicksForEpoch;
    }
    ...
    {
        _ticks = (static_cast<uint64>(info.tv_sec) * static_cast<uint64>(MicroSecondsPerSecond)) + static_cast<uint64>(info.tv_usec) + OffsetTicksForEpoch;
    ...
    Time::Time(const uint64 time, const bool localTime /*= false*/)
        : _time()
        , _ticks(time)
    {
        // This is the seconds since 1970...
    ...
    uint8 Time::DayOfWeek() const
    {
        return (static_cast<uint8>(_time.tm_wday));
    }

    uint16 Time::DayOfYear() const
    {
        return (static_cast<uint16>(_time.tm_yday));
    }
    ...
    Time& Time::Add(const uint32 timeInMilliseconds)
    {
        // Calculate the new time !!
        uint64 newTime = Ticks() + static_cast<uint64>(timeInMilliseconds) * MilliSecondsPerSecond;
        return (operator=(Time(newTime, IsLocalTime())));
    }

    Time& Time::Sub(const uint32 timeInMilliseconds)
    {
        // Calculate the new time !!
        uint64 newTime = Ticks() - static_cast<uint64>(timeInMilliseconds) * MilliSecondsPerSecond;
        return (operator=(Time(newTime, IsLocalTime())));
    ...
    {
        // static const uint64 epoch = 94354848000000000LL;

        // Tick count starts at 1st January 1970 00:00.00, so might need adaptions for NTP
        uint32 wholeSeconds = static_cast<uint32>(Ticks() / MicroSecondsPerSecond);
        wholeSeconds += NTPToUNIXSeconds;
        uint64 seconds = static_cast<uint64>(wholeSeconds) << 32;
        seconds += static_cast<uint32>(MilliSeconds() * 4294967.296); // 2^32/10^3

        return (seconds);
    }

sint32 Time::DifferenceFromGMTSeconds() const
{
#ifdef __WIN32__
    TIME_ZONE_INFORMATION timeZoneInfo;
    GetTimeZoneInformation(&timeZoneInfo);

    return static_cast<sint32>(-timeZoneInfo.Bias * 60);
#else
    return static_cast<sint32>(Handle().tm_gmtoff);
#endif
