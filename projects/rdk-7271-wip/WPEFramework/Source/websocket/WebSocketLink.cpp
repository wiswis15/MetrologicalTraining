...
namespace Web {
    namespace WebSocket {
        ...
        static const uint8 CONTINUATION_FRAME = 0x00;
        static const uint8 FINISHING_FRAME = 0x80;
        static const uint8 TYPE_FRAME = 0x0F;
        static const uint8 MASKING_FRAME = 0x80;
        static const uint8 CONTROL_FRAME = 0x08;
        static const uint8 HandShakeKey[] = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

        std::string Protocol::RequestKey() const
        {
            string baseEncodedKey;
            uint16 randomBytes[8];

            for (uint8 teller = 0; teller < 8; teller++) {
                Crypto::Random(randomBytes[teller]);
            }
            Core::ToString(reinterpret_cast<const uint8*>(&randomBytes[0]), sizeof(randomBytes), true, baseEncodedKey);
        ...
        std::string Protocol::ResponseKey(const std::string& requestKey) const
        {
            ...
            Crypto::SHA1 shaCalculator(reinterpret_cast<const uint8*>(requestKey.c_str()), static_cast<uint16>(requestKey.length()));
...
#endif
        uint16 Protocol::Encoder(uint8* dataFrame, const uint16 maxSendSize, const uint16 usedSize)
        {
            uint32 result = 0;
            ...
            if ((usedSize != 0) || (SendInProgress() == true)) {
                ...
                if ((_setFlags & MASKING_FRAME) == 0) {
                    ...
                    uint8 maskKey[4];
                    Crypto::Random(*reinterpret_cast<uint32*>(&maskKey[0]));

                    // Mask and insert the bytes on the right spots
                    uint8* source = &dataFrame[usedSize - 1 + 4];
                    uint8* destination = &dataFrame[usedSize - 1 + 4 + result];
                    uint32 bytesToMove = usedSize;
...
#pragma GCC diagnostic pop
        ...
        uint16 Protocol::Decoder(uint8* dataFrame, uint16& receivedSize)
        {
            uint16 actualHeader = 0;
            ...
            if (_pendingReceiveBytes > 0) {
                ...
                uint32 bytesToMove = (dataFrame[1] & 0x7F);
                ...
                if (actualHeader > receivedSize) {
                    ...
                    if ((_frameType & 0xF8) == 0) {
                        ...
                        if ((dataFrame[1] & MASKING_FRAME) != 0) {
                            ...
                            uint8* source = &dataFrame[actualHeader];
