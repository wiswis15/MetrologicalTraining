    ...
    namespace Web {
        ...
        bool MIMETypeForFile(const string path, string& fileToService, MIMETypes& mimeType)
        {
            ...
            if (filePresent == true) {
                ...
                if (position != -1) {
                    uint16 index = 0;
                    uint16 length = static_cast<uint16>(fileToService.length()) - (++position);
        ...
        uint16 Request::Serializer::Serialize(uint8 stream[], const uint16 maxLength)
        {
            uint16 current = 0;
                ...
                while ((current < maxLength) && (_state != REPORT)) {
                    ...
                    case PAIR_KEY: {
                        if (_buffer == NULL) {
                            if ((_keyIndex <= 0) && (_current->Host.IsSet())) {
                                ...
                                Core::NumberType<uint32, false, BASE_DECIMAL> number(_current->WebSocketVersion.Value());
                                ...
                                Core::NumberType<uint32, false, BASE_DECIMAL> number(_current->MX.Value());
                                number.Serialize(_value);
                                _offset = 0;
                            }
                                ...
                                Core::NumberType<uint32, false, BASE_DECIMAL> number(_bodyLength);
                    ...
                    case BODY: {
                        if (_bodyLength != 0) {
                            ...
                            uint32 size = (static_cast<uint32>(maxLength - current) <= _bodyLength ? static_cast<uint32>(maxLength - current) : _bodyLength);
        ...
        uint16 Response::Serializer::Serialize(uint8 stream[], const uint16 maxLength)
        {
            uint16 current = 0;
            ...
            if (_current != NULL) {
                while ((current < maxLength) && (_state != REPORT)) {
                    ...
                    case ERRORCODE: {
                        if (_buffer == NULL) {
                            Core::NumberType<uint32, false, BASE_DECIMAL> number(_current->ErrorCode);
                    ...
                    case PAIR_KEY: {
                        if (_buffer == NULL) {
                            if ((_keyIndex <= 0) && (_current->Server.IsSet())) {
                                ...
                                Core::NumberType<uint32, false, BASE_DECIMAL> number(_current->AccessControlMaxAge.Value());
        ...
        void Request::Deserializer::Parse(const uint8 stream[], const uint16 maxLength)
        {
            ...
            if (_current->_body.IsValid()) {
                ...
                if (_zlibResult == Z_OK) {
                    ...
                    _zlib.next_in = const_cast<uint8*>(stream);
                ...
                else if (_zlibResult == static_cast<uint32>(~0)) {
        ...
        void Request::Deserializer::Parse(const string& buffer)
        {
            ...
            case VERB: {
                ...
                if ((type.IsSet() == false) || ((_current = Element()) == NULL)) {
                    _parser.FlushLine();
                }
                else {
            ...
            case VERSION: {
                if ((buffer[0] == 'H') && (buffer[1] == 'T') && (buffer[2] == 'T') && (buffer[3] == 'P') && (buffer[4] == '/')) {
                    ...
                    uint32 usedChars = Core::Unsigned8::Convert(&(buffer.c_str()[5]), 3, number, BASE_DECIMAL);
                ...
                if (buffer.size() == 0) {
                    ...
                    if (chunked || ((_current->ContentLength.IsSet() == true) && (_current->ContentLength.Value() > 0))) {
                        ...
                        if ((_current->ContentEncoding.IsSet()) && (_current->ContentEncoding.Value() != EncodingTypes::ENCODING_UNKNOWN)) {
                            ...
                            _zlib.opaque = Z_NULL;
                            _zlib.avail_in = 0;
                            _zlib.next_in = Z_NULL;
                            _zlibResult = inflateInit2(&_zlib, 16 + MAX_WBITS);
                        }
                        else {
                            _zlibResult = static_cast<uint32>(~0);
                        }
            ...
            case PAIR_VALUE: {
                ...
                case Request::S_T:
                    ...
                    break;
                case Request::M_X:
                    _current->MX = Core::NumberType<uint32>(buffer.c_str(), buffer.length()).Value();
                    break;
            ...
            case CHUNK_INIT: {
                uint32 chunkedSize = Core::NumberType<uint32>(Core::TextFragment(buffer));
                if (chunkedSize == 0) {
                    _state = BODY_END;
                    _parser.FlushLine();
                }
        ...
        void Response::Deserializer::Parse(const uint8 stream[], const uint16 maxLength)
        {
            ASSERT(_current != NULL);
        ...
        void Response::Deserializer::Parse(const string& buffer)
        {
            ...
            case ERRORCODE: {
                Core::NumberType<uint16, false, BASE_DECIMAL> translatedCode(buffer.c_str(), buffer.size(), BASE_DECIMAL);
            ...
            case CHUNK_INIT: {
                uint32 chunkedSize = Core::NumberType<uint32>(Core::TextFragment(buffer), NumberBase::BASE_HEXADECIMAL);
                if (chunkedSize == 0) {
                    _parser.FlushLine();
                    _state = BODY_END;
                }
