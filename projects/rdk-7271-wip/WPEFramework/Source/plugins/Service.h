...
namespace PluginHost {
    ...
    private:
        ...
        public:
            ...
            Job(const Core::ProxyType<Core::IDispatchType<void> >& job)
                : _job(job)
            {
        ...
        typedef Core::ThreadPoolType<Job, 6> ThreadPool;
    ...
    private:
        ...
        WorkerPool(const uint32 stackSize);

    public:
        static WorkerPool& Instance(const uint32 stackSize = 0);
        ~WorkerPool();
    ...
    public:
        ...
        inline void Wait(const uint32 waitState, const uint32 time)
        {
            _workers.Wait(waitState, time);
        }
        inline void Submit(const Core::ProxyType<Core::IDispatchType<void> >& job)
        {
            _workers.Submit(Job(job), Core::infinite);
        }
        inline void Schedule(const Core::Time& time, const Core::ProxyType<Core::IDispatchType<void> >& job)
        {
            _timer.Schedule(time, TimedJob(job));
        }
        inline uint32 Revoke(const Core::ProxyType<Core::IDispatchType<void> >& job, const uint32 waitTime = Core::infinite)
        {
            ...
            return (_workers.Revoke(Job(job), waitTime));
        }
        inline void GetMetaData(MetaData::Server& metaData) const
        {
            ...
            for (uint8 teller = 0; teller < _workers.Count(); teller++) {
        ...
        public:
            ...
            inline void Update(const Plugin::Config& config)
            {
                ...
                if (_baseConfig.Accessor().PortNumber() == 80) {
                    ...
                    _accessor = string(_T("http://")) + _baseConfig.Accessor().HostAddress() + ':' + Core::NumberType<uint16>(_baseConfig.Accessor().PortNumber()).Text() + _webPrefix;
                }
            }
    ...
    public:
        ...
        virtual void* QueryInterface(const uint32 id) = 0;
        ...
        virtual uint32 Activate(const PluginHost::IShell::reason) = 0;
        virtual uint32 Deactivate(const PluginHost::IShell::reason) = 0;

        inline uint32 ConfigLine(const string& newConfiguration)
        {
...
#ifdef RUNTIME_STATISTICS
        uint32 _processedRequests;
        uint32 _processedObjects;
#endif
