...
ENUM_CONVERSION_BEGIN(PluginHost::VirtualInput::KeyMap::modifier)
    ...
    { PluginHost::VirtualInput::KeyMap::RIGHTSHIFT, _TXT("rigthshift") },
...
ENUM_CONVERSION_END(PluginHost::VirtualInput::KeyMap::modifier)
    ...
    namespace PluginHost {
       /*static */ VirtualInput* InputHandler::_inputHandler;
        ...
        uint32 VirtualInput::KeyMap::Load(const string& keyMap)
        {
            ...
            if ((keyMap.empty() == false) && (Core::File(keyMap).Exists() == true)) {
                ...
                std::map<uint16,uint16> previousKeys;
                std::map<uint16,sint16> updatedKeys;
                ...
                if (mappingFile.Open(true) == true) {
                    ...
                    while (index.Next() == true) {
                        if ((index.Current().Code.IsSet()) && (index.Current().Key.IsSet())) {
                            ...
                            if (_keyMap.find(code) == _keyMap.end()) {
                                ...
                                _keyMap.insert(std::pair<const uint32, const ConversionInfo>(code, conversionInfo));

                                std::map<uint16,uint16>::iterator index = previousKeys.find(conversionInfo.Code);
                    ...
                    std::map<uint16,uint16 >::const_iterator updatedKey(previousKeys.begin());
        ...
        uint32 VirtualInput::KeyMap::Save(const string& keyMap)
        {
            uint32 result = Core::ERROR_ILLEGAL_STATE;
            ...
            if (mappingFile.Create() == true) {
                ...
                std::map<const uint32, const ConversionInfo>::const_iterator index(_keyMap.begin());
                ...
                while (index != _keyMap.end()) {
                    ...
                    uint16 flag(1);
                    uint16 modifiers(index->second.Modifiers);
        ...
        uint32 VirtualInput::KeyEvent(const bool pressed, const uint32 code, const string& table)
        {
            uint32 result = Core::ERROR_UNKNOWN_TABLE;
            ...
            if (conversionTable != nullptr) {
                ...
                if ( (result == Core::ERROR_NONE) || (result == Core::ERROR_UNKNOWN_KEY_PASSED) ) {
                    ...
                    if ((pressed == false) && (sendModifiers != 0)) {
        ...
        void VirtualInput::Expired()
        {
        }

        void VirtualInput::RepeatKey(const uint32 code)
        {
            AdministerAndSendKey(REPEAT, code);
        }

        void VirtualInput::AdministerAndSendKey(const actiontype type, const uint32 code)
        {
            bool trigger;
        ...
        bool VirtualInput::SendModifier(const actiontype type, const enumModifier mode)
        {
            ...
            // or decrement.
            uint8 count((_modifiers >> mode) & 0xF);
        ...
        void VirtualInput::ModifierKey(const actiontype type, const uint16 modifiers)
        {
        ...
        /* virtual */ uint32 LinuxKeyboardInput::Open()
        {
        ...
        /* virtual */ uint32 LinuxKeyboardInput::Close()
        {
            if (_eventDescriptor != -1) {
        ...
        /* virtual */ void LinuxKeyboardInput::MapChanges(ChangeIterator& updated) {
            ...
            if (Updated(updated) == true) {
                ...
                if (_deviceKeys.size() > 0) {
                    ...
                    for_each(_deviceKeys.begin(), _deviceKeys.end(), [&](const std::pair<uint16, uint16>& key) {
        ...
        /* virtual */ void LinuxKeyboardInput::SendKey(const actiontype type, const uint32 code)
        {
        ...
        bool LinuxKeyboardInput::Updated(ChangeIterator& updated)
        {
            ...
            while (updated.Next() == true) {
                ...
                std::map<uint16, uint16>::iterator current(_deviceKeys.find(updated.Key()));
                if (current != _deviceKeys.end()) {
                    ...
                    updateTable = true;
                    _deviceKeys.insert(std::pair<uint16, uint16>(updated.Key(), *updated));
                }
            }
        ...
        /* virtual */ uint32 IPCKeyboardInput::Open()
        {
            return (_service.Open(2000));
        }

        /* virtual */ uint32 IPCKeyboardInput::Close()
        {
            _service.Close(2000);
            return (Core::ERROR_NONE);
        }

        /* virtual */ void IPCKeyboardInput::SendKey(const actiontype type, const uint32 code)
        {
