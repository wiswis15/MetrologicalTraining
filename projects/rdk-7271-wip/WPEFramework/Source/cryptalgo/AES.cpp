...
namespace WPEFramework {
	...
	uint32 AESEncryption::Key(const uint8 length, const uint8 key[]) {
	...
	uint32 AESEncryption::Encrypt(const uint32 length, const uint8 input[], uint8 output[]) {
		...
		case AES_CFB128:
		{
			uint32 delta = (16 - _offset) % 16;

			if (((delta <= length) && ((_offset + length) >= 16))) {

				uint32 blockSize = (((length - _offset) / 16) * 16) + delta;
				// First encrypt the whole blocks. No padding needed, yet 
	...
	uint32 AESDecryption::Key(const uint8 length, const uint8 key[]) {
	...
	uint32 AESDecryption::Decrypt(const uint32 length, const uint8 input[], uint8 output[]) {
		uint32 result = Core::ERROR_UNAVAILABLE;
