...
namespace Crypto {
    ...
    public:
            ...
            : _algorithm()
        {
            ...
            if (key.length() > sizeof(_innerKeyPad)) {
                ...
                hashKey.Input(reinterpret_cast<const uint8*>(key.c_str()), static_cast<uint16>(key.length()));
                encryptionKey = hashKey.Result();
            }
            else {
                keyLength = static_cast<uint8>(key.length());

                // Use the key as is..
                encryptionKey = reinterpret_cast<const uint8*>(key.c_str());
            }
            ...
            for (uint8 index = 0; index < keyLength; index++) {
    ...
    public:
        ...
        static const uint8 Length = HASHALGORITHM::Length;
        inline static uint8 BlockLength()
        {
        ...
        {
            if (_computed == false) {
                uint8 hashKey[HASHALGORITHM::Length];
         ...
         */
        inline void Input(const uint8 message_array[], const uint16 length)
        {
            _algorithm.Input(message_array, length);
        }

        inline HMACType<HASHALGORITHM>& operator<<(const uint8 message_array[])
        {
            uint16 length = 0;
        ...
        inline HMACType<HASHALGORITHM>& operator<<(const uint8 message_element)
        {
    ...
    private:
        bool _computed;
        uint8 _innerKeyPad[64];
        uint8 _outerKeyPad[64];
