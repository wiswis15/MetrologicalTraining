...
int GetDeviceId(unsigned short MaxIdLength, char Id[])
{
    ...
    if (_channel.Open (1000) == Core::ERROR_NONE) { // Wait for 1 Second.
        ...
        Core::ProxyType<Core::IIPC> message (Core::proxy_cast<WPEFramework::Core::IIPC>(_deviceId));
        uint32 error  = _channel.Invoke (message, Core::infinite);

        result = -error;

        if (error == WPEFramework::Core::ERROR_NONE)
        {
...
int GetDRMId(const char label[], const unsigned short MaxIdLength, char Id[])
{
    ...
    if (_channel.Open (1000) == Core::ERROR_NONE) { // Wait for 1 Second.
        ...
        Core::ProxyType<Core::IIPC> message (Core::proxy_cast<WPEFramework::Core::IIPC>(_drmId));
