...
#pragma warning(default : 4355)
        ...
        const ITrace* information)
    {
        ...
        ASSERT(cleanClassName.Length() < (sizeof(m_Output._traceBuffer) - static_cast<uint32>(result - m_Output._traceBuffer)));

        memcpy(result, cleanClassName.Data(), cleanClassName.Length());
        result = &(result[cleanClassName.Length()]);
        *result++ = '\0';

		uint16 length = static_cast<uint16>(result - m_Output._traceBuffer);
		uint16 bufferRemaining = sizeof(m_Output._traceBuffer) - length;
		uint16 copiedBytes = (bufferRemaining > information->Length()) ? information->Length() : bufferRemaining;
    ...
    void TraceMedia::HandleMessage(const uint8* dataFrame, const uint16 receivedSize)
    {
        if ((receivedSize >= 6) && (dataFrame[0] == 'C')) {
            ...
            case 0: { // List all trace categories !!
                ...
                while (index.Next() == true) {
                    ...
                    Write(reinterpret_cast<const uint8*>(response), static_cast<uint32>(sizeof(response)) - size);
                }
                break;
            }
            case 1: { // Toggle tracing
                ...
                if (module != NULL) {
                    uint32 count = TraceUnit::Instance().SetCategories((dataFrame[6] == '1'), &module[1], reinterpret_cast<const char*>(&dataFrame[8]));
                    ...
                    Write(reinterpret_cast<const uint8*>(response), 10);
                }
                break;
            }
            }
    ...
    void TraceMedia::Write(const uint8* dataFrame VARIABLE_IS_NOT_USED, const uint16 receivedSize VARIABLE_IS_NOT_USED)
    {
    }

    char* TraceMedia::CopyText(char* destination, const char* source, uint32 maxSize)
    {
        uint32 size = maxSize - 1;
        const char* end = reinterpret_cast<const char*>(::memchr(source, '\0', size));

        if (end != NULL) {
            size = static_cast<uint32>(end - source);
        }
