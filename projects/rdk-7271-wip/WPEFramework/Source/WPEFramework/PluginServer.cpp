...
#ifdef __WIN32__
/* static */ const TCHAR* Server::ConfigFile = _T("C:\\Projects\\WPEFrameworkConfig.json");
#else
/* static */ const TCHAR* Server::ConfigFile = _T("/etc/WPEFramework/config.json");
#endif
...
static Core::NodeId DetermineAccessor(const Server::Config& configuration, Core::NodeId& accessor) {
    ...
    if (configuration.Interface.Value().empty() == false) {
        ...
        Core::IPV4AddressIterator selectedNode = interfaces.IPV4Unicast(configuration.Interface.Value());

        if (selectedNode.IsValid() == true) {
            accessor = selectedNode.Address();
            result = accessor;
        }
    ...
    if (accessor.IsValid() == false) {
        TRACE_L1("Invalid config information could not resolve to a proper IP (%d)", 0);
    } else {
        ...
        URL += ':' + Core::NumberType<uint32>(configuration.Port.Value()).Text();
...
void Server::ServiceMap::GetMetaData(Core::JSON::ArrayType<MetaData::Service>& metaData) const {
    _adminLock.Lock();
    ...
    while (index != _services.end()) {
        ...
        Core::ProxyType<const Service> pluginInfo(index->second);

        ASSERT (pluginInfo.IsValid() == true);

        pluginInfo->GetMetaData(newInfo);
...
void Server::ServiceMap::Destroy() {
    ...
    TRACE_L1("Deactivating %d plugins.", static_cast<uint32>(_services.size()));
    ...
    TRACE_L1("Destructing %d plugins.", static_cast<uint32>(_services.size()));
    ...
    std::list<uint32> pidList;

    // See if there are still pending processes, we need to shoot and kill..
    Processes(pidList);

    if (pidList.size() > 0) {
        uint16 waitSlots = 50; /* each slot is 100ms, so we wait for 5 Seconds for all processes to terminte !!! */

        std::list<uint32>::iterator index(pidList.begin());
        ...
        while ((waitSlots-- != 0) && (pidList.size() > 0)) {
            ...
            std::list<uint32>::iterator check(pidList.begin());
...
/* virtual */ void* Server::Service::QueryInterface(const uint32 id) {
...
/* virtual */ void* Server::Service::QueryInterfaceByCallsign(const uint32 id, const string& name) {
...
/* virtual */ void* Server::Service::Instantiate(const uint32 waitTime, const string& className, const uint32 interfaceId, const uint32 version, uint32& pid, const string& locator) {
...
/* virtual */ RPC::IRemoteProcess* Server::Service::RemoteProcess(const uint32 pid) {
    return (_administrator.RemoteProcess(pid));
}

// Methods to stop/start/update the service.
uint32 Server::Service::Activate(const PluginHost::IShell::reason why) {
...
uint32 Server::Service::Deactivate(const reason why) {
...
Core::ProxyType<PluginHost::Server::Service> Server::ServiceMap::FromLocator(const string& identifier, bool& correctHeader) {
	...
	if ((correctHeader == true) && (identifier.length() > (locator.length() + 1))) {
		uint32 length;
		uint32 offset = locator.length() + 1; /* skip the slash after */
...
Server::Server(Server::Config& configuration, ISecurity* securityHandler, const bool background)
    ...
    _controller->ClassType<Plugin::Controller>()->Started();
...
/* virtual */ void Server::Connectivity(const Core::NodeId& /* externalIP */) {
}
