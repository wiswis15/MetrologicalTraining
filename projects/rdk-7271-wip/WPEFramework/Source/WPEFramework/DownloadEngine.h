...
namespace WPEFramework {
	namespace PluginHost {
		...
		public:
			uint32 Start(const string& locator, const string& destination, const uint8 hash[Crypto::HASH_SHA256])
			{
				Core::URL url(locator);
				uint32 result = (url.IsValid() == true ? Core::ERROR_INPROGRESS : Core::ERROR_INCORRECT_URL);
			...
			virtual void Transfered(const uint32 result, const string& source, const string& destination) = 0;

		private:
			virtual void Transfered(const uint32 result, const Web::SignedFileBodyType<Crypto::SHA256>& destination) {
		...
		private:
			struct DownloadInfo {
				string _source;
				string _destination;
				uint8 _hash[Crypto::HASH_SHA256];
			} _current;
