...
namespace WPEFramework {
	namespace Plugin {
		...
		private:
			...
			public:
				inline WebTransform() : _keywordLength(static_cast<uint8>(_tcslen(Web::Request::MSEARCH)))
				{
				}
				inline ~WebTransform()
				{
				...
				uint16 Transform(Web::Request::Deserializer& deserializer, uint8* dataFrame, const uint16 maxSendSize);

			private:
				uint8 _keywordLength;
			};
			...
			private:
				...
				Core::ProxyType<Web::Response> CreateResponse(const uint64 incomingTime, const uint64 requestTime);
			...
			public:
				Broadcaster(Probe& parent, const uint8 timeToLive) :
			...
			public:
				void Ping(const Core::NodeId& destination, const uint8 timeToLive) {
					if (IsOpen()) {
						Destination newEntry;
				...
				virtual uint16 SendData(uint8* dataFrame, const uint16 maxSendSize) {
					...
					else
					{
						...
						if ((TTL(info._timeToLive) == Core::ERROR_NONE) && ((data = CreateRequest()).empty() == false)) {
							...
							size = (maxSendSize > data.length() ? static_cast<uint8>(data.length()) : maxSendSize);
			...
			private:
				std::string CreateRequest() {
					string text;
					string sendTimerInUS(Core::NumberType<uint64>(Core::Time::Now().Ticks()).Text());
					*_body = sendTimerInUS;
		...
		public:
			...
			public:
				...
				Instance(const Core::URL& url, const uint32 latency, const string& model, const bool secure)
					: _URL(url)
					, _latency(latency)
					, _model(model)
					, _secure(secure)
		...
		public:
			...
			#endif
			Probe(const string& responseURL, const uint8 timeToLive, const string& model) :
				_adminLock(),
				_partners(),
		...
		public:
			inline void Ping(const uint8 timeToLive = 0) {
		...
		private:
			...
			void Report(const Core::URL& remote, const uint32 latencyinUs, const string& model, const bool secure);
