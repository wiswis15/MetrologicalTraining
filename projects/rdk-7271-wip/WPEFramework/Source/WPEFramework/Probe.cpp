...
namespace WPEFramework {
	namespace Plugin {
		...
		public:
			...
			Discovery (const Core::URL& remote, const uint32 latencyinUs) 
                        {
				_text = "Discovered: [" + remote.Text().Text() + "] with latency: " + 
                                        Core::NumberType<uint32>(latencyinUs).Text() + " uS";
			}
			Discovery (const uint32 latencyinUs) 
                        {
				_text = "Internal adjustment latency: [" + 
                                        Core::NumberType<uint32>(latencyinUs).Text() + "] uS";
			}
		...
		uint16 Probe::WebTransform::Transform(Web::Request::Deserializer& deserializer, uint8* dataFrame, const uint16 maxSendSize)
		{
			uint16 index = 0;
			const uint8* checker = dataFrame;
		...
		{
			uint64 incomingTime = Core::Time::Now().Ticks();
			Core::NodeId sourceNode(Link().ReceivedNode());

			if ((request->Verb == Web::Request::HTTP_MSEARCH) && (request->ST.IsSet() == true)) {
				if ((Core::NodeId(sourceNode, Interface.PortNumber()) != Link().LocalNode()) &&
					(request->ST.Value() == Probe::SearchTarget) &&
					(request->HasBody() == true) && (ValidRequest(*request) == true)) {
					string body(*(request->Body<Web::TextBody>()));
					Destination newEntry;
					newEntry._destination = sourceNode;
					newEntry._requestTime = Core::NumberType<uint64>(body.c_str(), body.length()).Value();
		...
		bool Probe::Listener::ValidRequest(const Web::Request& request) const {
			...
			if ((request.ContentSignature.IsSet() == true) && (request.ContentSignature.Value().Type() == Crypto::HASH_SHA256))
			{
				...
				bool valid = (validateBlob(
                                            ...
                                            Crypto::HASH_SHA256, 
                                            request.ContentSignature.Value().Data()) == 0);
				if (valid == false) {
		...
		Core::ProxyType<Web::Response> Probe::Listener::CreateResponse(const uint64 incomingTime, const uint64 requestTime)
		{
			...
			uint32 delta   = static_cast<uint32>(Core::NumberType<uint64>(Core::Time::Now().Ticks()) - incomingTime);
			uint64 newTime = requestTime + delta;
			string newBody = Core::NumberType<uint64>(newTime).Text();
...
#if defined(SECURITY_LIBRARY_INCLUDED)
			uint8 base64Buffer[Crypto::HASH_SHA256];
		...
		/* virtual*/ uint16 Probe::Broadcaster::ReceiveData(uint8* dataFrame, const uint16 receivedSize) {

			uint64 stamp(Core::NumberType<uint64>(Core::Time::Now().Ticks()));
			...
			if ((response.ApplicationURL.IsSet() == true) && (ValidResponse(response) == true)) {
				...
				uint64 latency;
                                string model;
                                uint32 length = _answer->find(' ', 0);
                                bool secure = ((response.ContentSignature.IsSet() == true) && (response.ContentSignature.Value().Type() == Crypto::HASH_SHA256));

                                if (length == string::npos) {
				    latency = Core::NumberType<uint64>(_answer->c_str(), _answer->length()).Value();
                                }
                                else {
				    latency = Core::NumberType<uint64>(_answer->c_str(), length).Value();
                                    model = _answer->substr(length + 1, string::npos);
                                }
				latency = (stamp - latency);

				if (latency < 0xFFFFFFFF) {
					_parent.Report(response.ApplicationURL.Value(), static_cast<uint32>(latency), model, secure);
				}
                                else {
		...
		bool Probe::Broadcaster::ValidResponse(const Web::Response& response) const {
			...
			if ((response.ContentSignature.IsSet() == true) && (response.ContentSignature.Value().Type() == Crypto::HASH_SHA256))
			{
				...
				bool valid = (validateBlob(
                                    		 ...
                                    		 reinterpret_cast<const unsigned char*>(value.c_str()), 
                                      		 Crypto::HASH_SHA256, 
                               			 response.ContentSignature.Value().Data()) == 0);
				if (valid == false) {
	   			         TRACE(Discovery, (string(_T("Received an invalid Discovery request."))));
				}
                       		return (valid);
		...
		void Probe::Report(const Core::URL& remote, const uint32 latencyinUs, const string& model, const bool secure) {
			_adminLock.Lock();
