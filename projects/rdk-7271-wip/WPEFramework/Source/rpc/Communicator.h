#ifndef __RPC_PROCESSLAUNCH_H
#define __RPC_PROCESSLAUNCH_H
...
namespace RPC {
    ...
    struct EXTERNAL IRemoteProcess : virtual public Core::IUnknown {
        ...
        virtual void* Instantiate(const uint32 waitTime, const string& className, const uint32 interfaceId, const uint32 version) = 0;
        virtual void Terminate() = 0;

        template <typename REQUESTEDINTERFACE>
        REQUESTEDINTERFACE* Instantiate(const uint32 waitTime, const string& className, const uint32 version)
        {
    ...
    public:
        ...
        private:
			...
			RemoteProcess(RemoteProcessMap* parent, uint32 pid, const Core::ProxyType<Core::IPCChannel>& channel)
				: _parent(parent)
				, _process(pid)
			...
			RemoteProcess(RemoteProcessMap* parent, uint32* pid, const Core::Process::Options* options)
                : _parent(parent)
                , _process(false)
        ...
        public:
            inline static RemoteProcess* Create(RemoteProcessMap& parent, const uint32 pid, Core::ProxyType<Core::IPCChannel>& channel)
            {
			...
			inline static RemoteProcess* Create(RemoteProcessMap& parent, uint32& pid, const Core::Process::Options& options)
			{
        ...
        public:
            ...
            virtual void* Instantiate(const uint32 waitTime, const string& className, const uint32 interfaceId, const uint32 version);

            uint32 WaitState(const uint32 state, const uint32 time) const
            {
            ...
            void Terminate();
            void Announce(Core::ProxyType<Core::IPCChannel>& channel, const uint32 exchangeId, const uint32 interfaceId, void* implementation);
    ...
    private:
        ...
        private:
            ...
            public:
                uint64 Timed(const uint64 scheduledTime)
                {
                    uint64 result = 0;
        ...
        private:
            ...
            static constexpr uint32 DestructionStackSize = 64 * 1024;
        ...
        public:
			inline void Register(RPC::IRemoteProcess::INotification* sink)
			{
				...
				if (sink != nullptr) {
					...
					std::map<uint32, RemoteProcess* >::iterator index(_processes.begin());
            ...
            {
                return (static_cast<uint32>(_processes.size()));
            }
            inline Communicator::RemoteProcess* Create(const Core::Process::Options& options, uint32& pid)
            {
                _adminLock.Lock();
                ...
                if (result != nullptr) {
                    ...
                    _processes.insert(std::pair<uint32, RemoteProcess* >(result->Id(), result));
                }
            ...
            inline void Destroy(const uint32 pid)
            {
                // First do an activity check on all processes registered.
                _adminLock.Lock();

                std::map<uint32, Communicator::RemoteProcess* >::iterator index(_processes.find(pid));
            ...
            Communicator::RemoteProcess* Process(const uint32 id)
            {
                Communicator::RemoteProcess* result = nullptr;

                _adminLock.Lock();

                std::map<uint32, Communicator::RemoteProcess* >::iterator index(_processes.find(id));
            ...
            inline void Processes(std::list<uint32>& pidList) const
            {
                // First do an activity check on all processes registered.
                _adminLock.Lock();

                std::map<uint32, Communicator::RemoteProcess* >::const_iterator index(_processes.begin());
            ...
            uint32 Announce(Core::ProxyType<Core::IPCChannel>& channel, const uint32 exchangeId, const uint32 interfaceId, void* implementation)
            {
                ...
                if (channel.IsValid() == true) {
                    ...
                    std::map<uint32, Communicator::RemoteProcess* >::iterator index(_processes.find(exchangeId));
                    ...
                    if (index != _processes.end()) {
                        ...
                        if (remoteProcess != nullptr) 
                        {
                            ...
                            // insertion :-)
                            _processes.insert(std::pair<uint32, Communicator::RemoteProcess* > (exchangeId, remoteProcess));
        ...
        private:
            ...
            std::map<uint32, Communicator::RemoteProcess* > _processes;
        ...
        public:
            ...
            void Link(RemoteProcessMap& processMap, const uint32 pid)
            {
        ...
        private:
            inline uint32 Announce(Core::IPCChannel& channel, const uint32 exchangeId, const uint32 interfaceId, void* implementation)
            {
                Core::ProxyType<Core::IPCChannel> baseChannel(channel);

                ASSERT(baseChannel.IsValid() == true);

                uint32 result = _processes.Announce(baseChannel, exchangeId, interfaceId, implementation);
    ...
    public:
		...
		inline void CreateFactory(const uint32 initialSize)
		{
        ...
        inline Communicator::RemoteProcess* Process(const uint32 id)
        {
            return (_processMap.Process(id));
        }
		inline Communicator::RemoteProcess* Create(uint32& pid, const Core::Process::Options& options) {
    ...
    public:
        ...
        inline INTERFACE* Create(const string& className, const uint32 version = static_cast<uint32>(~0), const uint32 waitTime = CommunicationTimeOut)
        {
            return (static_cast<INTERFACE*>(Create(waitTime, className, INTERFACE::ID, version)));
        }
        uint32 Open(const uint32 waitTime = CommunicationTimeOut);
		uint32 Open(const uint32 interfaceId, void* implementation, const uint32 waitTime = CommunicationTimeOut);
		uint32 Close(const uint32 waitTime);
		void WaitForCompletion();

    private:
		void* Create(const uint32 waitTime, const string& className, const uint32 interfaceId, const uint32 version = static_cast<uint32>(~0));
