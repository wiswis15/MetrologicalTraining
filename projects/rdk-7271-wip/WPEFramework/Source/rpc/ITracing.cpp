...
namespace ProxyStub {
	...
	ProxyStub::MethodHandler RemoteProcessStubMethods[] = {
		[](Core::ProxyType<Core::IPCChannel>& channel VARIABLE_IS_NOT_USED, Core::ProxyType<RPC::InvokeMessage>& message) {
			...
			response.Number<uint32>(message->Parameters().Implementation<RPC::IRemoteProcess>()->Id());
		},
		[](Core::ProxyType<Core::IPCChannel>& channel VARIABLE_IS_NOT_USED, Core::ProxyType<RPC::InvokeMessage>& message) {
		...
		[](Core::ProxyType<Core::IPCChannel>& channel VARIABLE_IS_NOT_USED, Core::ProxyType<RPC::InvokeMessage>& message) {
			...
			uint32 waitTime(parameters.Number<uint32>());
			string className(parameters.Text());
			uint32 interfaceId(parameters.Number<uint32>());
			uint32 version(parameters.Number<uint32>());
