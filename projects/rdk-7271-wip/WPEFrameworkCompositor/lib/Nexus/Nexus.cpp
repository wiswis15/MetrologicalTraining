...
namespace Plugin {
    ...
    private:
        ...
        public:
            ...
            virtual void Focus() override
            {
                ...
                // NOTE: In our system, we have a dedicated nameless IR InputClient,
                // It is sharable between all other clients.
                // So far, we do not need to filter IR router to be handled only by the current focused client
                // nxserver_p_focus_input_client(_client);

                /* bring first SurfaceClient to top of zorder */
                nxserver_p_focus_surface_client(_client);
            }
            virtual void Opacity(const uint32 value) override
            {
                ASSERT(_client != NULL);
            ...
            virtual void Geometry(const uint32 X, const uint32 Y, const uint32 width, const uint32 height) override
            {
                // TODO
            }
        ...
        public:
            void Initialize(const uint32 sleepTime)
            {
    ...
    public:
        ...
        /* virtual */ uint32 Configure(const string& Config)
        {
            ...
            if (_hardwarestate != Exchange::IResourceCenter::FAILURE) {
                ...
                uint32 sleep = config.HWDelay.Value();
    ...
    public:
        ...
        /* virtual */ Exchange::IComposition::IClient* Client(const uint8 id)
        {
            uint8 count = id;
