...
public:
    ...
    public:
        Config() :
                Sleep(90), Crash(false), Destruct(1000) {
            Add(_T("sleep"), &Sleep);
            Add(_T("crash"), &Crash);
    ...
    public:
        enum runtype {
            SHOW, HIDE, RESUMED, SUSPENDED
        };
        Job() :
                _parent(nullptr), _type(SHOW) {
        }
        Job(OutOfProcessImplementation& parent, const runtype type) :
                _parent(&parent), _type(type) {
        }
        Job(const Job& copy) :
                _parent(copy._parent), _type(copy._type) {
        }
        ~Job() {
        }
...
public:
    OutOfProcessImplementation() :
            Core::Thread(0, _T("OutOfProcessImplementation")), _reference(), _requestedURL(), _setURL(), _fps(0), _hidden(false), _executor(0, _T("TestPool")) {
    }
...
public:
    virtual void SetURL(const string& URL) {
        ...
        TRACE_L1("URL length: %lu", URL.length());
    ...
    virtual void Register(PluginHost::IStateControl::INotification* sink) {
        ...
        TRACE_L1("IStateControl::INotification Registered in outofprocessimpl: %p", sink);
        _adminLock.Unlock();
    }
    ...
    virtual void Register(Exchange::IBrowser::INotification* sink) {
        ...
        TRACE_L1("IBrowser::INotification Registered in outofprocessimpl: %p", sink);
        _adminLock.Unlock();
    }
