...
namespace Plugin {
  ...
  public:
      ...
      virtual uint32_t Configure(const string& path, int interval, const string& mode, const string& parentName)
      {
         uint32_t result(Core::ERROR_INCOMPLETE_CONFIG);
         ...
         Config config;
         //config.Path = "/tmp/resource-log.bin";
         config.Path = path;
         //config.Interval = 5;
         config.Interval = interval;
         //config.Mode = "single";
         config.Mode = mode;
         //config.ParentName = "WPEFramework-1.0.0";
